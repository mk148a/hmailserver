...
The .NET 10 branch is a side-by-side rewrite and is not a production release. Code/test commits `89cc2c860` and `e5b441b36` close retained `GlobalObjects`/`DeliveryQueue` and root `Links` authorization gaps. Legacy COM authorization is rechecked at property/method entry; the queue preserves `Clear` HRESULT `0x800403E9`, queue `S_FALSE` branches, and `Links` preserves the standard access-denied message/HRESULT. Focused coverage for each slice is `8 passed, 0 failed, 0 skipped`; the latest full run is `1944 passed, 0 failed, 31 skipped`.

The offline 100,000-message SEARCH/SORT benchmark passes, but it is not evidence of live SQL FTS, protocol, concurrency, C++ equivalence, or soak acceptance. The next code candidate is authorization propagation to child facades returned by `Links`; the approved disposable SQL/Data restore acceptance remains the highest-priority environment-gated slice.
