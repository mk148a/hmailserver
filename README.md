...
## Current parity gate (2026-08-22, legacy POP3 authenticated command gating)

Code/test commit `e4334f5fd` closes the legacy POP3 authenticated command-gating
gap. Legacy `GetCommand` at `source/Server/POP3/POP3Connection.cpp:172-206`
rejects USER/PASS during TRANSACTION with `-ERR Invalid command in current
state.`; Net10 now does the same. Focused POP3/listener tests pass `23`, and
full Debug Net10 passes `2696`, skips `94`, and fails `0` (`2790` total).
...
SQL/Data round-trip acceptance. The previous empty-USER entry below is
historical.
historical.
