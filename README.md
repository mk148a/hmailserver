...
The .NET 10 branch is a side-by-side rewrite and is not a production release. Code/test commit `52d000029` extends retained `Links` authorization into directly returned Domain, Alias, DistributionList, and Account facades, following legacy `InterfaceLinks::{get_Domain,get_Account,get_Alias,get_DistributionList}` and child `SetAuthentication` behavior. The root and direct-child guards preserve installed COM identity and deny retained child reads/mutations after failed reauthentication without additional store reads. Focused Links/related coverage is `78 passed, 0 failed, 0 skipped`; the latest full run is `1945 passed, 0 failed, 31 skipped`.

The offline 100,000-message SEARCH/SORT benchmark passes, but it is not evidence of live SQL FTS, protocol, concurrency, C++ equivalence, or soak acceptance. The next code candidate is retained authorization through Account `Messages`/`FetchAccounts`/`Rules`/`IMAPFolders`, plus nested Domain collections; the approved disposable SQL/Data restore acceptance remains the highest-priority environment-gated slice.
