...
The .NET 10 branch is a side-by-side rewrite and is not a production release. Code/test commit `5542ced99` closes retained-child authorization gaps for Message `Attachments`/`Recipients` and Rule `Criterias`, following legacy `InterfaceMessage`/`InterfaceRule` authentication propagation while preserving installed COM identity. Focused Message/RuleCriteria coverage is `60 passed, 0 failed, 0 skipped`; the latest filtered full run is `1945 passed, 0 failed, 31 skipped`. The default full run remains `1947 passed, 2 failed, 31 skipped`, with both failures caused by host AV locking generated scanner `.eml` files during cleanup.

The offline 100,000-message SEARCH/SORT benchmark passes, but it is not evidence of live SQL FTS, protocol, concurrency, C++ equivalence, or soak acceptance. Legacy `InterfaceMessage::get_Headers` does not attach the shared authentication object, so Message Headers remain intentionally unchanged in this slice. The next code candidate is nested Domain collection authorization, followed by approved disposable SQL/Data restore acceptance; scanner AV cleanup remains an environment-specific test blocker.
