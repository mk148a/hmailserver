...
The .NET 10 branch is a side-by-side rewrite and is not a production release. Code/test commit `1bbffd74d` closes three retained-object gaps: direct `Account.ID` now enforces attachment, and retained `FetchAccount`/`Message` draft setters recheck authentication before staging. The slice follows legacy `InterfaceAccount::SetAuthentication` and preserves installed COM identity. Focused Account/FetchAccount/Message coverage is `81 passed, 0 failed, 0 skipped`; the latest filtered full run is `1944 passed, 0 failed, 31 skipped`. The default full run remains `1947 passed, 2 failed, 31 skipped`, with both failures caused by host AV locking generated scanner `.eml` files during cleanup.

The offline 100,000-message SEARCH/SORT benchmark passes, but it is not evidence of live SQL FTS, protocol, concurrency, C++ equivalence, or soak acceptance. The next code candidate is retained grandchild authorization for `Message.Attachments`/`Recipients`/`Headers` and `Rule.Criterias`, followed by nested Domain collections; the approved disposable SQL/Data restore acceptance remains the highest-priority environment-gated slice.
