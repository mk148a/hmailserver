...
The .NET 10 branch is a side-by-side rewrite and is not a production release. Code/test commit `9b93e9c34` extends retained `Links` Account authorization through `AdminLevel`, `Messages`, `FetchAccounts`, `Rules`, and folder `Messages`, following legacy `InterfaceAccount::SetAuthentication` and nested getters. Focused Account/Links coverage is `123 passed, 0 failed, 0 skipped`; the latest default full run is `1947 passed, 2 failed, 31 skipped`, where both failures are AV-locked scanner cleanup tests, and excluding those scanner classes gives `1942 passed, 0 failed, 31 skipped`.

The offline 100,000-message SEARCH/SORT benchmark passes, but it is not evidence of live SQL FTS, protocol, concurrency, C++ equivalence, or soak acceptance. The next code candidate is retained-child negative coverage for `Rules`, `IMAPFolders`, and draft setter paths, followed by nested Domain collections; the approved disposable SQL/Data restore acceptance remains the highest-priority environment-gated slice.
