...
The .NET 10 branch is a side-by-side rewrite and is not a production release. Code/test commit `44e41839f` closes retained child-facade authorization gaps for `GroupMember.Account` and `IMAPFolderPermission.Account/Group`, including the `Settings.PublicFolders` snapshot route. Legacy `InterfaceGroupMember::get_Account`, `InterfaceIMAPFolderPermission::get_Account/get_Group`, `InterfaceSettings::get_PublicFolders`, `InterfaceIMAPFolders` item factories, and `InterfaceIMAPFolder::get_Permissions` all attach the shared authentication object; the .NET path now carries the live callback through each corresponding factory. Focused GroupMember/IMAP coverage is `32 passed, 0 failed, 0 skipped`; filtered full Net10 is `1948 passed, 0 failed, 31 skipped`.

The default full run is `1953 passed, 2 failed, 31 skipped`. Both failures are `UnauthorizedAccessException` cleanup failures in `ClamWinScannerTestRuntimeTests` and `CustomScannerTestRuntimeTests`, where host AV locks generated `.eml` files. The offline 100,000-message SEARCH/SORT benchmark remains diagnostic only, not live SQL FTS, protocol, concurrency, C++ equivalence, or soak evidence. The next code slice is a fresh parity audit of remaining COM/Admin child-facade paths; approved disposable SQL/Data restore remains the highest-priority environment-gated work.
