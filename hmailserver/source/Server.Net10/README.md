...
Message queue identity metadata (`ID`, `UID`, `DeliveryAttempt`, and `InternalDate`) also follows the legacy read-only COM shape. VBScript rejects direct assignment, while JScript restores canonical seeded values at `Load`, `Save`, and `Copy` boundaries; 64-bit message IDs remain intact. `State` remains separate because the legacy indexed `Flag(eMessageFlag)` setter intentionally mutates its bits.
