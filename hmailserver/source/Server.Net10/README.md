...
full suite is `2434 passed, 10 skipped, 0 failed` (`2444` total).
...
normalization. Net10 now matches the legacy retry/flag defaults and uses the SQL
equivalent of `PersistentIMAPFolder::GetUniqueMessageID` for zero-UID mailbox
messages. No COM identity, SMTP behavior, shared IMAP/COM read behavior, or
installed registration changed.

Next slice: implement target-preexisting group dependency parity with explicit
unresolved-group and restore rollback coverage. Paired C++/.NET performance,
...
participant adapter. Focused SMTP/participant coverage is `9/9`; full Net10
Debug is `2305 passed, 57 skipped, 0 failed`. All protocol hosted services now
use the helper; readiness, restore, and COM wiring remain open.
...
Debug is `2305 passed, 57 skipped, 0 failed`. POP3, SMTP, readiness, restore,
and COM wiring remain open.
...
- The .NET `DistributionListRecipients` adapter and SQL administration store now implement the bounded authenticated Add/Save, existing-row Save, and Delete parity slice with generation-bound authorization leases; direct child activation remains `E_ACCESSDENIED`, while authenticated Settings read access and SQL parameterization remain unchanged. SMTP list-policy behavior, live reconfiguration, service/database/Data-directory state, and SEC-18 staging state did not change. Broader Admin collections and live WebAdmin request execution remain separate release work.
...
`ApplicationComClass.Reinitialize` remain open; paired C++/.NET performance
