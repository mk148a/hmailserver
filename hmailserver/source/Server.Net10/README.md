...
- `HMailServer.Storage.SqlServer`: SQL Server connection, Full-Text Search readiness, message search/sort indexing, IMAP sequence snapshots, IMAP message fetch storage, POP3 Inbox mailbox storage, external fetch account/UID leasing, failed-logon auto-ban recording, atomic delivery leasing, read-only domain/account administration lookup, optional greylisting checks, optional delivery queue status persistence, retention cleanup, and event-kind metrics snapshots.
