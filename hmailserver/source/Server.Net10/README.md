...
- `HMailServer.ComInterop`: the preserved legacy `IInterfaceMessageIndexing` IID/dual/DISPID contract plus additive COM compatibility contracts for new .NET-only capabilities. The project builds a portable contract assembly for reflection tests and a `net10.0-windows` COM-hosting assembly; runtime class activation and Administrator object-model wiring remain on the parity backlog.
