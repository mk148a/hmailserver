...
The caller-source and in-memory broker-only AppID preflight gates are implemented in .NET 10, and `WindowsWebAdminBrokerRegistryEvidenceSource` now captures both 32-bit and 64-bit broker/legacy AppID views through read-only registry handles. The preflight projector requires readable byte-unchanged legacy Application AppID snapshots, one matching readable broker snapshot per registry view, explicit ordinary DACL ACEs for the worker/service SIDs with the required local DCOM access mask, and no deny, inherited, callback, inherit-only, or object ACEs. Focused tests cover missing/read-error/conflicting views, wrong legacy paths, malformed/non-binary values, exact ACE/mask rejection, unchanged snapshots, and name/kind/byte changes. The next bounded SEC-18 slice is complete installed Application CLSID/ProgID/CurVer/LocalServer32/TypeLib registration-graph readback and byte/key preservation evidence; only after that evidence is independently reviewed should isolated COM integration proceed. Do not register the broker, change registry/IIS/service configuration, edit PHP, alter `IInterfaceApplication`, persist tokens or passwords, access production data, or change SMTP/IMAP/POP3 behavior.
