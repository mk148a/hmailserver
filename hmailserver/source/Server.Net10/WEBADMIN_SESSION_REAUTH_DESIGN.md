...
The caller-source and in-memory broker-only AppID preflight gates are implemented in .NET 10, and `WindowsWebAdminBrokerRegistryEvidenceSource` now captures both 32-bit and 64-bit broker views plus the complete installed Application CLSID/ProgID/CurVer/LocalServer32/TypeLib/Interface graph through read-only registry handles. The preflight projector requires readable graph snapshots, exact key-path/presence/value-name/value-kind/native-byte equality before and after, one matching readable broker snapshot per registry view, explicit ordinary DACL ACEs for the worker/service SIDs with the required local DCOM access mask, and no deny, inherited, callback, inherit-only, or object ACEs. The graph verifier now preserves the installed Registry32 asymmetry where all six Application CLSID-subtree keys are absent, rejects unexpected direct descendants, and validates canonical fixed values plus installation-specific executable/HELPDIR path relationships. Focused graph/preflight tests pass 20/20; the full Net10 suite passes 1122/1122. Legacy graph identity evidence is anchored to `InterfaceApplication.rgs`, `hMailServer.rgs`, `hMailServer.idl`, `hMailServer.cpp`, and `hMailServer.rc`. Read-only live graph evidence was captured on 2026-07-18, but both independent security and reality reviews remain RED because key-ACL capture, checked-in collector attestation, and native-reader integration tests are still missing. The next bounded SEC-18 slice is the fresh authorized/non-pool matrix with one shared invocation ID, followed by collector-issued challenge/final-state sealing, external manifest pinning, and fresh independent review before isolated COM integration. Do not register the broker, change registry/IIS/service configuration, edit PHP, alter `IInterfaceApplication`, persist tokens or passwords, access production data, or change SMTP/IMAP/POP3 behavior.
