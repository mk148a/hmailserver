...
The caller-source and in-memory broker-only AppID preflight gates are implemented in .NET 10, and `WindowsWebAdminBrokerRegistryEvidenceSource` now captures both 32-bit and 64-bit broker/legacy AppID views through read-only registry handles while preserving exact value kinds and raw bytes. Focused tests cover missing broker registration, dual-view capture, unchanged legacy AppID snapshots, and name/kind/byte changes. The next bounded SEC-18 slice is isolated COM integration plus feeding this readback into the preflight gate: prove authorized PHP-worker activation, unauthorized local activation/method denial, bearer-token reuse denial, configured-SID-change denial, exact broker AppID LaunchPermission/AccessPermission descriptors, and byte-for-byte preservation of the existing Application registration. Do not register the broker, change registry/IIS/service configuration, edit PHP, alter `IInterfaceApplication`, persist tokens or passwords, access production data, or change SMTP/IMAP/POP3 behavior.
