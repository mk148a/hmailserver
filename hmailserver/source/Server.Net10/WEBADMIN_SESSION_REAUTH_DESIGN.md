...
The caller-source and in-memory broker-only AppID preflight gates are implemented in .NET 10, and `WindowsWebAdminBrokerRegistryEvidenceSource` now captures both 32-bit and 64-bit broker views plus the complete installed Application CLSID/ProgID/CurVer/LocalServer32/TypeLib/Interface graph through read-only registry handles. The preflight projector requires readable graph snapshots, exact key-path/presence/value-name/value-kind/native-byte equality before and after, one matching readable broker snapshot per registry view, explicit ordinary DACL ACEs for the worker/service SIDs with the required local DCOM access mask, and no deny, inherited, callback, inherit-only, or object ACEs. The graph intentionally preserves the installed Registry32 asymmetry where the Application CLSID root is absent. Focused graph/preflight tests pass 17/17 and the full Net10 suite passes 1118/1118. Legacy graph identity evidence is anchored to `InterfaceApplication.rgs`, `hMailServer.rgs`, `hMailServer.idl`, `hMailServer.cpp`, and `hMailServer.rc`. Read-only live graph evidence was captured on 2026-07-18, but both independent security and reality reviews returned RED because canonical expected contents, recursive unknown-subkey/key-ACL detection, checked-in collector attestation, and native-reader integration tests are still missing. The next bounded SEC-18 slice is the deterministic checked-in canonical graph verifier and collector before isolated COM integration. Do not register the broker, change registry/IIS/service configuration, edit PHP, alter `IInterfaceApplication`, persist tokens or passwords, access production data, or change SMTP/IMAP/POP3 behavior.
