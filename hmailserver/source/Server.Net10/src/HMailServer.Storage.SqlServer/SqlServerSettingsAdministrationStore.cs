...
""";
    ...
    public const string UpdateServicePop3Sql = """
UPDATE hm_settings
SET settinginteger = @ServicePOP3
...
""";
    ...
    public const string UpdateImapSortEnabledSql = """
UPDATE hm_settings
SET settinginteger = @IMAPSortEnabled
...
""";
    ...
    public const string UpdateImapIdleEnabledSql = """
UPDATE hm_settings
SET settinginteger = @IMAPIdleEnabled
WHERE settingname = N'enableimapidle';
""";

    public const string UpdateImapAclEnabledSql = """
UPDATE hm_settings
SET settinginteger = @IMAPACLEnabled
...
""";
    ...
    public const string UpdateImapSaslInitialResponseEnabledSql = """
UPDATE hm_settings
SET settinginteger = @IMAPSASLInitialResponseEnabled
...
""";
    ...
    public const string UpdateMaxInvalidLogonAttemptsSql = """
UPDATE hm_settings
SET settinginteger = @MaxInvalidLogonAttempts
...
""";
    ...
    public const string UpdateAutoBanMinutesSql = """
UPDATE hm_settings
SET settinginteger = @AutoBanMinutes
...
""";
    ...
    public const string UpdateAntiVirusClamWinExecutableSql = """
UPDATE hm_settings
SET settingstring = @AntiVirusClamWinExecutable
WHERE settingname = N'avclamwinexec';
""";

    public const string UpdateAntiVirusClamWinDatabaseSql = """
UPDATE hm_settings
SET settingstring = @AntiVirusClamWinDatabase
...
""";
    ...
    public const string UpdateAntiVirusNotifyReceiverSql = """
UPDATE hm_settings
SET settinginteger = @AntiVirusNotifyReceiver
WHERE settingname = N'avnotifyreceiver';
""";

    public const string UpdateAntiVirusNotifySenderSql = """
UPDATE hm_settings
SET settinginteger = @AntiVirusNotifySender
...
""";
    ...
    public const string UpdateAntiVirusCustomScannerReturnValueSql = """
UPDATE hm_settings
SET settinginteger = @AntiVirusCustomScannerReturnValue
...
""";
    ...
    public const string UpdateAntiVirusClamAvHostSql = """
UPDATE hm_settings
...
""";
    ...
    public const string UpdateAntiSpamGreyListingInitialDelaySql = """
UPDATE hm_settings
...
""";
    ...
    public const string UpdateAntiSpamGreyListingFinalDeleteSql = """
UPDATE hm_settings
...
""";
    ...
    public const string UpdateBackupSettingsSql = """
UPDATE hm_settings
SET settinginteger = (settinginteger & ~@BackupSettingsMask) | @BackupSettingsValue
WHERE settingname = N'backupoptions';
""";

    public const string UpdateBackupDomainsSql = """
UPDATE hm_settings
SET settinginteger = (settinginteger & ~@BackupDomainsMask) | @BackupDomainsValue
WHERE settingname = N'backupoptions';
""";
...
""";
    ...
    public const string UpdateBackupCompressionSql = """
UPDATE hm_settings
...
""";
    ...
    public async ValueTask<bool> UpdateMaxInvalidLogonAttemptsAsync(
    ...
    public async ValueTask<bool> UpdateAutoBanMinutesAsync(
        int autoBanMinutes,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateAntiVirusClamWinExecutableAsync(
        string executable,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using var command = new SqlCommand(UpdateAntiVirusClamWinExecutableSql, connection);
        command.Parameters.Add("@AntiVirusClamWinExecutable", SqlDbType.NVarChar, 4000).Value = executable;

        return await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false) == 1;
    }

    public async ValueTask<bool> UpdateAntiVirusClamWinDatabaseAsync(
        string database,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateAntiVirusNotifyReceiverAsync(
        bool notifyReceiver,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using var command = new SqlCommand(UpdateAntiVirusNotifyReceiverSql, connection);
        command.Parameters.Add("@AntiVirusNotifyReceiver", SqlDbType.Int).Value = notifyReceiver ? 1 : 0;

        return await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false) == 1;
    }

    public async ValueTask<bool> UpdateAntiVirusNotifySenderAsync(
        bool notifySender,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateAntiVirusCustomScannerReturnValueAsync(
        int returnValue,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateAntiVirusClamAvHostAsync(
        string host,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateServicePop3Async(
        bool servicePop3,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateImapSortEnabledAsync(
    ...
    public async ValueTask<bool> UpdateImapIdleEnabledAsync(
        bool imapIdleEnabled,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using var command = new SqlCommand(UpdateImapIdleEnabledSql, connection);
        command.Parameters.Add("@IMAPIdleEnabled", SqlDbType.Int).Value = imapIdleEnabled ? 1 : 0;

        return await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false) == 1;
    }

    public async ValueTask<bool> UpdateImapAclEnabledAsync(
        bool imapAclEnabled,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateImapSaslInitialResponseEnabledAsync(
    ...
    public async ValueTask<bool> UpdateAntiSpamGreyListingInitialDelayAsync(
        int minutes,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateAntiSpamGreyListingFinalDeleteAsync(
        int hours,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using var command = new SqlCommand(UpdateAntiSpamGreyListingFinalDeleteSql, connection);
        command.Parameters.Add("@GreyListingFinalDelete", SqlDbType.Int).Value = hours;
    ...
    public async ValueTask<bool> UpdateBackupSettingsAsync(
        bool backupSettings,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using var command = new SqlCommand(UpdateBackupSettingsSql, connection);
        command.Parameters.Add("@BackupSettingsMask", SqlDbType.Int).Value = 1;
        command.Parameters.Add("@BackupSettingsValue", SqlDbType.Int).Value = backupSettings ? 1 : 0;

        return await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false) == 1;
    }

    public async ValueTask<bool> UpdateBackupDomainsAsync(
        bool backupDomains,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async ValueTask<bool> UpdateBackupCompressionAsync(
        bool backupCompression,
        CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenAsync(cancellationToken).ConfigureAwait(false);
        await using var command = new SqlCommand(UpdateBackupCompressionSql, connection);
        command.Parameters.Add("@BackupCompressionMask", SqlDbType.Int).Value = 8;
        command.Parameters.Add("@BackupCompressionValue", SqlDbType.Int).Value = backupCompression ? 8 : 0;
