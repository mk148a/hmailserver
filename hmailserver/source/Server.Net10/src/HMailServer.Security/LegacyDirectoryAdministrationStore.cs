...
public sealed class LegacyDirectoryAdministrationStore : IDirectoryAdministrationStore
{
    ...
    public ValueTask<bool> UpdateProgramDirectoryAsync(
        string programDirectory,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(programDirectory);
        cancellationToken.ThrowIfCancellationRequested();
        return ValueTask.FromResult(LegacyInitializationFile.SaveProgramDirectory(_initializationFile, programDirectory));
    }

    public ValueTask<bool> UpdateEventDirectoryAsync(
        string eventDirectory,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(eventDirectory);
        cancellationToken.ThrowIfCancellationRequested();
        return ValueTask.FromResult(LegacyInitializationFile.SaveEventDirectory(_initializationFile, eventDirectory));
    }

    public ValueTask<bool> UpdateDatabaseDirectoryAsync(
        string databaseDirectory,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(databaseDirectory);
        cancellationToken.ThrowIfCancellationRequested();
        return ValueTask.FromResult(LegacyInitializationFile.SaveDatabaseDirectory(_initializationFile, databaseDirectory));
    }
