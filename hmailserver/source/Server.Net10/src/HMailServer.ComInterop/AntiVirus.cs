...
public sealed class AntiVirus : IInterfaceAntiVirus
{
    ...
    private readonly Action<bool>? _publishClamAvEnabled;
    private readonly Action<string>? _publishClamAvHost;
    private readonly Action<int>? _publishClamAvPort;
    ...
    private AntiVirus(
        ...
        Action<bool>? publishClamAvEnabled,
        Action<string>? publishClamAvHost,
        Action<int>? publishClamAvPort)
    {
        _snapshot = snapshot;
        ...
        _publishClamAvEnabled = publishClamAvEnabled;
        _publishClamAvHost = publishClamAvHost;
        _publishClamAvPort = publishClamAvPort;
    }
    ...
    {
        get => Snapshot.CustomScannerExecutable;
        set
        {
            _ = Snapshot;
            if (_settingsMutationStore is null)
            ...
            if (!_settingsMutationStore
                .UpdateAntiVirusCustomScannerExecutableAsync(value, CancellationToken.None)
                .GetAwaiter()
                .GetResult())
            {
                throw new COMException(
                    "The custom scanner executable update did not affect the existing settings row.",
                    EFail);
            }

            if (_snapshot is not null)
            {
                _snapshot = _snapshot with { CustomScannerExecutable = value };
            }

            _publishCustomScannerExecutable?.Invoke(value);
        }
    }
    ...
    public bool ClamAVEnabled
    {
        ...
        set
        {
            ...
            if (!_settingsMutationStore
                .UpdateAntiVirusClamAvEnabledAsync(value, CancellationToken.None)
                .GetAwaiter()
                .GetResult())
            {
                throw new COMException(
                    "The ClamAV enabled update did not affect the existing settings row.",
                    EFail);
            }

            if (_snapshot is not null)
            {
                _snapshot = _snapshot with { ClamAvEnabled = value };
            }

            _publishClamAvEnabled?.Invoke(value);
        }
    }

    public string ClamAVHost
    {
        get => Snapshot.ClamAvHost;
        set
        {
            _ = Snapshot;
            if (_settingsMutationStore is null)
            ...
            if (!_settingsMutationStore
                .UpdateAntiVirusClamAvHostAsync(value, CancellationToken.None)
                .GetAwaiter()
                .GetResult())
            {
                throw new COMException(
                    "The ClamAV host update did not affect the existing settings row.",
                    EFail);
            }

            if (_snapshot is not null)
            {
                _snapshot = _snapshot with { ClamAvHost = value };
            }

            _publishClamAvHost?.Invoke(value);
        }
    }

    public int ClamAVPort
    {
        get => Snapshot.ClamAvPort;
        set
        {
            _ = Snapshot;
            if (_settingsMutationStore is null)
            ...
            if (!_settingsMutationStore
                .UpdateAntiVirusClamAvPortAsync(value, CancellationToken.None)
                .GetAwaiter()
                .GetResult())
            {
                throw new COMException(
                    "The ClamAV port update did not affect the existing settings row.",
                    EFail);
            }

            if (_snapshot is not null)
            {
                _snapshot = _snapshot with { ClamAvPort = value };
            }

            _publishClamAvPort?.Invoke(value);
        }
    }
    ...
    internal static AntiVirus CreateAuthorized(
        ...
        Action<bool>? publishClamAvEnabled = null,
        Action<string>? publishClamAvHost = null,
        Action<int>? publishClamAvPort = null)
    {
