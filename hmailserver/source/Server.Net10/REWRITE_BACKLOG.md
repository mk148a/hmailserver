...
Continue legacy Administrator parity with the next bounded SMTP guardrail slice: expose read-only `Settings.MaxMessageSize`, `MaxSMTPRecipientsInBatch`, `DisconnectInvalidClients`, and `MaxNumberOfInvalidCommands` from the existing `maxmessagesize`, `maxsmtprecipientsinbatch`, `disconnectinvalidclients`, and `maximumincorrectcommands` `hm_settings.settinginteger` rows. Preserve installed vtable/DISPIDs and `VARIANT_BOOL` marshaling; keep setters, live SMTP session/listener policy changes, and broader Settings/Admin behavior out of scope.
