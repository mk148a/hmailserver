...
Continue legacy Administrator parity with the next bounded crash-simulation Settings slice: expose read-only `Settings.CrashSimulationMode` from the process-local runtime value, which starts at the legacy default `0`. Preserve installed DISPID 99 and integer marshaling; keep the setter, SMTP crash/fault injection, exception-handler test behavior, and broader Settings/Admin mutation out of scope.
