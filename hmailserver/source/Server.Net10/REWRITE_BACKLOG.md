...
Wake the hosted delivery worker immediately after each successful durable SMTP queue commit. Inject the existing coalescing signal into the SQL queue writer through service DI, publish only after the message/recipient/unlock transaction commits, cover primary and rule-generated messages, and prove failed or rolled-back enqueues do not signal. Preserve SMTP acceptance, SQL shape, file cleanup, lease/retry behavior, and keep `DeliveryQueue.Clear`/`Remove` outside this slice.
