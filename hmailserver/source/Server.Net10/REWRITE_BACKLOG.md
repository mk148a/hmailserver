...
Continue legacy Administrator parity with the next bounded `Logging.Directory` slice: expose authenticated `Settings -> Logging.Directory` as a read-only value from the configured/default legacy `hMailServer.ini` log directory setting. Keep the existing `Logging` contract/registration/auth boundary unchanged and leave setters, live logging, current-log paths/file reads, logger reconfiguration, and broader Settings/Admin mutation out of scope.
