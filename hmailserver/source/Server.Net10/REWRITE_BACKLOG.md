...
Continue the Administrator object model with a bounded read-only `Group -> Members` slice: preserve the legacy `GroupMembers`/`GroupMember` COM contracts, expose authenticated Group count/index/id lookup plus read-only `ID`, `GroupID`, and `AccountID` scalar values from existing `hm_group_members` rows ordered by member ID, and keep the Account child facade, ACL runtime behavior, Add/Delete/Save, mutations, and deeper behavior explicit `E_NOTIMPL`.
