...
Continue legacy Administrator parity with the next bounded UI-language Settings slice: expose read-only `Settings.UserInterfaceLanguage` from the configured/default legacy `hMailServer.ini` `[Settings] UseLanguage` value, preserving the legacy `English` default, installed DISPID 42, and `BSTR` marshaling. Keep the setter, INI persistence, Administrator UI resource reload, and broader Settings/Admin mutation out of scope.
