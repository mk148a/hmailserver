...
Continue legacy Administrator parity with the next smallest safe slice: expose `Domain.Size` through an authenticated, read-only SQL aggregate after first locking down the legacy SQL behavior with narrow contract/store/integration tests. Preserve the installed vtable/DISPIDs; keep quota enforcement, account mutation, filesystem/mailbox scans, and broader domain mutation behavior out of scope.
