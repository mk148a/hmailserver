...
Resume the Administrator object model with a bounded read-only `Application -> Status` slice: preserve the legacy `Status` COM identity/vtable contract, expose authenticated in-process status scalars and counters (`UndeliveredMessages`, `StartTime`, processed/removed counters, `SessionCount`, and `ThreadID`) behind a runtime snapshot boundary, and keep direct activation plus unavailable live-runtime details explicit `E_ACCESSDENIED`/`E_NOTIMPL`.
