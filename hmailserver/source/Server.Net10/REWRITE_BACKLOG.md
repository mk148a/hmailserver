...
Continue legacy Administrator parity with the next smallest safe slice: expose the non-secret Active Directory `Account` scalars (`IsAD`, `ADDomain`, and `ADUsername`) through authenticated, read-only getters backed by existing `hm_accounts` columns. Preserve the installed vtable/DISPIDs; keep setters, Active Directory authentication, password/security-sensitive fields, account mutation, and broader account behavior out of scope.
