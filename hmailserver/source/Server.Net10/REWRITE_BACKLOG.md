...
Continue legacy Administrator parity with the next smallest safe slice: expose `Account.QuotaUsed` through an authenticated, read-only computed scalar using the selected account's `hm_messages.messagesize` byte total and `accountmaxsize` MB limit, matching legacy integer percentage behavior. Preserve the installed vtable/DISPIDs; keep quota enforcement, account mutation, filesystem/mailbox scans, and broader account behavior out of scope.
