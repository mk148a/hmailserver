...
Continue DKIM as an evaluation-only verifier core by adding a message-level orchestration boundary: extract one or more `DKIM-Signature` fields plus the canonical header/body inputs from a message source, run parser + DNS/public-key lookup + body-hash + header-signature verification in legacy-safe order, and return `Pass` when any signature verifies. Keep SMTP rejection, policy scoring, signing, and Administrator settings out of this slice.
