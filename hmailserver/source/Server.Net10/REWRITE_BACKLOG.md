...
Continue legacy Administrator parity with the next bounded TLS bitmask Settings slice: expose read-only `Settings.TlsVersion10Enabled`, `TlsVersion11Enabled`, `TlsVersion12Enabled`, `TlsVersion13Enabled`, `TlsOptionPreferServerCiphersEnabled`, and `TlsOptionPrioritizeChaChaEnabled` from existing `SslVersions` and `TlsOptions` `hm_settings.settinginteger` bitmasks. Preserve installed DISPIDs 96/97/98/103/105/106 and `VARIANT_BOOL` marshaling; map legacy bits 2/4/8/16 for TLS versions and 2/4 for TLS options; keep setters, live TLS reload/listener context rebuild, cipher validation, OS TLS policy changes, and broader Settings/Admin mutation out of scope.
