...
Continue legacy Administrator parity with the next bounded Settings network preference slice: expose read-only `Settings.IPv6PreferredEnabled` from the existing `IPv6Preferred` `hm_settings.settinginteger` row. Preserve installed vtable/DISPID 104 and `VARIANT_BOOL` marshaling; keep setters, socket/listener resolver preference changes, live network reconfiguration, and broader Settings/Admin mutation out of scope.
