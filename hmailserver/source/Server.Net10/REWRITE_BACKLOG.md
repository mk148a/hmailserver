...
Continue legacy Administrator parity with the next smallest safe slice: expose `Account.Size` through an authenticated, read-only SQL aggregate over the selected account's `hm_messages.messagesize` values, matching legacy `AccountSizeCache` behavior by returning a 3-decimal MB float. Preserve the installed vtable/DISPIDs; keep `QuotaUsed`, quota enforcement, account mutation, filesystem/mailbox scans, and broader account behavior out of scope.
