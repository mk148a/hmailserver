...
Continue the Administrator object model with a bounded read-only `Settings -> Groups` slice: preserve the legacy `Groups`/`Group` COM contracts, expose authenticated Settings count/index/name/id lookup plus read-only `ID` and `Name` scalar values from existing `hm_groups` rows ordered by group name, and keep group members, ACL behavior integration, Add/Delete/Save, mutations, and deeper behavior explicit `E_NOTIMPL`.
