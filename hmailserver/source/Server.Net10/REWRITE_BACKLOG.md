...
6. COM/API compatibility.
   ...
   - Done: expose read-only `Settings.RuleLoopLimit`, `WorkerThreadPriority`, `TCPIPThreads`, and `MaxNumberOfMXHosts` from their existing numeric runtime rows, preserving DISPIDs 48/57/60/90 and integer metadata while leaving setters, live rule-loop/thread tuning, SMTP MX scheduling, and `MaxAsynchronousThreads` out of scope pending separate storage verification.
...
Continue legacy Administrator parity with the next bounded asynchronous-thread Settings slice: expose read-only `Settings.MaxAsynchronousThreads` from existing `MaxNumberOfAsynchronousTasks` `hm_settings.settinginteger` data. Preserve installed DISPID 88 and integer marshaling; keep the setter, live work-queue resizing, service restart/reconfiguration behavior, and broader Settings/Admin mutation out of scope.
