...
Continue legacy Administrator parity with a bounded `Settings -> BackupSettings` slice: replace the placeholder with the complete legacy IID/vtable/class identity and hosted registration, expose authenticated read-only `Destination` plus option bits 1/2/4/8 from existing `backupdestination`/`backupoptions` rows, and expose deterministic `LogFile` from the configured log directory. Preserve direct-activation denial; keep all setters, backup/restore execution, filesystem writes, runtime reconfiguration, and broader Settings/Admin mutation out of scope.
