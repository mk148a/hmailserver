...
Continue legacy Administrator parity with the next bounded IMAP naming slice: expose read-only `Settings.IMAPPublicFolderName` and `IMAPHierarchyDelimiter` from the existing `imappublicfoldername` and `IMAPHierarchyDelimiter` `hm_settings.settingstring` rows. Preserve installed vtable/DISPIDs 74/87 and `BSTR` marshaling; keep setters, folder/rule rename-rewrite, live IMAP namespace changes, and broader Settings/Admin behavior out of scope.
