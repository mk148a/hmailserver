...
Continue legacy Administrator parity with the next smallest safe slice: expose `Account.LastLogonTime` through an authenticated, read-only getter backed by the existing `hm_accounts.accountlastlogontime` value. Preserve the installed vtable/DISPIDs and legacy VARIANT date shape; keep login-time updates, authentication behavior, account mutation, and broader account behavior out of scope.
