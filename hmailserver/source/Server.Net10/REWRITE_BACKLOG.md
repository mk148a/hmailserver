...
Resume the Administrator object model with a bounded read-only `Application -> Database` slice: preserve the legacy `Database` COM identity/vtable contract, expose only authenticated non-mutating database/version/configuration scalars that can be proven from the existing SQL/runtime configuration, and keep direct activation plus SQL execution, transaction, database creation/default-selection, script execution, message filename utility, prerequisite, and persistence operations explicit `E_ACCESSDENIED`/`E_NOTIMPL`.
