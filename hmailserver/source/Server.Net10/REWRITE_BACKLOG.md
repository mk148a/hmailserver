...
Continue legacy Administrator parity with the next bounded SMTP policy slice: expose read-only `Settings.AllowSMTPAuthPlain`, `DenyMailFromNull`, `AllowIncorrectLineEndings`, and `AddDeliveredToHeader` from the existing `authallowplaintext`, `allowmailfromnull`, `smtpallowincorrectlineendings`, and `adddeliveredtoheader` `hm_settings.settinginteger` rows. Preserve the legacy inversion of `DenyMailFromNull`, installed vtable/DISPIDs 8/11/61/73, and `VARIANT_BOOL` marshaling; keep setters, live SMTP authentication/session/delivery changes, and broader Settings/Admin behavior out of scope.
