...
Continue legacy Administrator parity with a bounded `Application -> GlobalObjects -> DeliveryQueue` contract slice: replace the `GlobalObjects` placeholder and add the complete legacy `DeliveryQueue` IID/vtable/class identities plus hosted registration, expose the authenticated child graph, and preserve direct-activation denial. Keep `Clear`, `ResetDeliveryTime`, `StartDelivery`, `Remove`, language collections, SQL mutation, worker wake-up, and broader queue behavior explicit `E_NOTIMPL` until the runtime/store boundary lands separately.
