...
Continue queue administration parity with a bounded `DeliveryQueue.ResetDeliveryTime` slice: add an authenticated SQL Server administration boundary that updates only the selected type-1/type-3 `hm_messages` row to `messagetype = 1` with an immediately eligible next-try timestamp, preserving the 64-bit message ID and leaving retry count, content files, recipients, leases, queue clear/remove, and worker wake-up unchanged. Keep direct activation denied and the other queue methods `E_NOTIMPL`.
