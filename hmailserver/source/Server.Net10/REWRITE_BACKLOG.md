...
Continue legacy Administrator parity with the next smallest safe slice: expose `Domain.AllocatedSize` through an authenticated, read-only SQL aggregate over the selected domain's `hm_accounts.accountmaxsize` values, matching legacy MSSQL MB semantics. Preserve the installed vtable/DISPIDs; keep `Domain.Size`, quota enforcement, account mutation, filesystem/mailbox scans, and broader domain mutation behavior out of scope.
