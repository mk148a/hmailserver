...
Continue legacy Administrator parity with the next bounded IMAP master-user Settings slice: expose read-only `Settings.IMAPMasterUser` from existing `ImapMasterUser` `hm_settings.settingstring` data. Preserve installed DISPID 100 and `BSTR` marshaling; keep the setter, IMAP authentication/master-user behavior, live IMAP configuration reload, secret handling, and broader Settings/Admin mutation out of scope.
