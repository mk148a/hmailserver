...
Continue legacy Administrator parity with the next smallest safe slice: expose bounded read-only `Settings` integer limits (`MaxSMTPConnections`, `MaxPOP3Connections`, `MaxIMAPConnections`, and `MaxDeliveryThreads`) from existing `hm_settings.settinginteger` rows. Preserve the installed vtable/DISPIDs and legacy setting names; keep setters, live listener/delivery-worker reconfiguration, service state changes, and broader Settings/Admin behavior out of scope.
