...
Continue legacy Administrator parity with the next smallest safe slice: expose a bounded read-only `Settings` string scalar set (`HostName`, `WelcomeSMTP`, `WelcomePOP3`, and `WelcomeIMAP`) from existing `hm_settings` rows. Preserve the installed vtable/DISPIDs; keep setters, listener reconfiguration, service state changes, secret settings, and broader Settings/Admin behavior out of scope.
