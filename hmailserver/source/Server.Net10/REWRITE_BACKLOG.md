...
Continue the Administrator object model with a bounded read-only `Settings -> SecurityRanges` slice: preserve the legacy `SecurityRanges`/`SecurityRange` COM contracts, expose authenticated Settings count/index/name/id lookup plus core read-only IP range, priority, expiry, and option scalar values from existing `hm_securityranges` rows, and keep IP policy enforcement, auto-ban runtime behavior, `SetDefault`, Add/Delete/Save, mutations, and deeper behavior explicit `E_NOTIMPL`.
