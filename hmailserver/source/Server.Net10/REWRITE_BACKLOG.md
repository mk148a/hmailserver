...
Continue legacy Administrator parity with the next bounded IMAP SASL slice: expose read-only `Settings.IMAPSASLPlainEnabled` and `IMAPSASLInitialResponseEnabled` from the existing `EnableImapSASLPlain` and `EnableImapSASLInitialResponse` `hm_settings.settinginteger` rows. Preserve installed vtable/DISPIDs 101/102 and `VARIANT_BOOL` marshaling; keep setters, live IMAP authentication/capability changes, and broader Settings/Admin behavior out of scope.
