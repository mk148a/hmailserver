...
Continue legacy Administrator parity with the next bounded Settings SSL/TLS scalar slice: expose read-only `Settings.VerifyRemoteSslCertificate` and `SslCipherList` from the existing `VerifyRemoteSslCertificate` `hm_settings.settinginteger` row and `SslCipherList` `hm_settings.settingstring` row. Preserve installed vtable/DISPIDs 93/94 and `VARIANT_BOOL`/`BSTR` marshaling; keep setters, live TLS listener/client reload, cipher validation, certificate policy changes, TLS version flags, and broader Settings/Admin mutation out of scope.
