...
Continue the Administrator object model with a bounded read-only `Settings -> IncomingRelays` slice: preserve the legacy `IncomingRelays`/`IncomingRelay` COM contracts, expose authenticated Settings count/index/name/id lookup plus read-only `ID`, `Name`, `LowerIP`, and `UpperIP` scalars from `hm_incoming_relays` rows ordered by `relayname asc`, and keep SMTP trust behavior, Add/Delete/Save, mutations, and deeper behavior explicit `E_NOTIMPL`.
