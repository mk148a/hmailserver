...
Resume the Administrator object model with a bounded read-only `Application` core-scalar slice: preserve legacy `ServerState`, `Version`, and `InitializationFile` COM DISPIDs/return shapes behind runtime/configuration boundaries, keep service-control operations (`Start`, `Stop`, `Connect`, `Reinitialize`, `SubmitEMail`) explicit `E_NOTIMPL` until their side effects are fully bounded, and avoid widening child object mutation scope.
