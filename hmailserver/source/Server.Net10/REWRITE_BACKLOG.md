...
5. Security and anti-abuse modernization.
   ...
   - Done: DKIM evaluation-only foundation with the legacy result model (`Neutral`, `Pass`, `TempFail`, `PermFail`), deterministic `DKIM-Signature` tag parser, required-field validation, default/simple/relaxed canonicalization selection, signed-header list parsing, `b=` signature-value blanking, simple/relaxed body/header canonicalization tests, `bh=` body-hash verification over canonicalized body data including optional `l=` body-length handling and SHA1/SHA256 hash selection, and injected-public-key header cryptographic verification for `rsa-sha1`/`rsa-sha256`. Full DKIM verification only models `Pass` after both body hash and header signature verification succeed; mismatches, impossible body lengths, invalid public keys, and bad signatures map to `PermFail`. This does not perform DNS selector/public-key lookup, SMTP rejection, policy scoring, signing, or Administrator setting wiring.
   - Remaining: DKIM DNS/public-key lookup and key-record validation, DKIM policy/Admin wiring, DMARC; later Administrator/COM setting parity for SPF/greylisting knobs.
...
Continue DKIM as an evaluation-only verifier core by adding DNS/public-key lookup behind an injected resolver boundary: query `{selector}._domainkey.{domain}` TXT records, parse/validate the DKIM key record (`v=DKIM1`, non-empty `p=`, compatible `h=` when present, revoked-key handling), and feed the resulting public key into the existing body-hash + header-signature verifier. Keep SMTP rejection, policy scoring, signing, and Administrator settings out of this slice.
