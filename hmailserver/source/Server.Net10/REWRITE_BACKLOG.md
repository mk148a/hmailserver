...
Continue the Administrator object model with a bounded read-only `Settings -> TCPIPPorts` slice: preserve the legacy `TCPIPPorts`/`TCPIPPort` COM contracts, expose authenticated Settings count/index/id lookup plus read-only `ID`, `Protocol`, `PortNumber`, `Address`, `UseSSL`, `SSLCertificateID`, and `ConnectionSecurity` scalars from `hm_tcpipports` rows ordered by `portaddress1 asc, portaddress2 asc, portnumber asc`, and keep listener reconfiguration, `SetDefault`, Add/Delete/Save, mutations, and deeper behavior explicit `E_NOTIMPL`.
