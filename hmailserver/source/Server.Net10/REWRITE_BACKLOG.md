...
Continue legacy Administrator parity with the next bounded non-secret SMTP relayer Settings slice: expose read-only `Settings.SMTPRelayer`, `SMTPRelayerRequiresAuthentication`, `SMTPRelayerUsername`, `SMTPRelayerPort`, and `SMTPRelayerConnectionSecurity` from existing `smtprelayer`, `usesmtprelayerauthentication`, `smtprelayerusername`, `smtprelayerport`, and `smtprelayerconnectionsecurity` `hm_settings` rows. Preserve installed vtable/DISPIDs and `BSTR`/`VARIANT_BOOL`/integer/enum marshaling; keep `SetSMTPRelayerPassword`, `smtprelayerpassword`, legacy `SMTPRelayerUseSSL` compatibility, setters, credential handling, live SMTP delivery routing changes, and broader Settings/Admin mutation out of scope.
