...
Continue legacy Administrator parity with the next smallest safe slice: expose existing `hm_domains` signature configuration through authenticated, read-only `Domain` COM getters. Preserve the installed vtable/DISPIDs and enum values; keep setters, message mutation, and SMTP policy changes out of scope.
