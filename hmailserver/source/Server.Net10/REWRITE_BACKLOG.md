...
Continue legacy Administrator parity with the next bounded current-log path slice: expose authenticated read-only `Logging.CurrentEventLog`, `CurrentErrorLog`, `CurrentAwstatsLog`, and `CurrentDefaultLog` using the configured log directory and legacy filename/date shapes. Keep the existing `Logging` contract/registration/auth boundary unchanged, perform no file reads, and leave live logging, setters, logger reconfiguration, and broader Settings/Admin mutation out of scope.
