...
Continue legacy Administrator parity with the next smallest safe slice: expose bounded read-only `Settings.SMTPNoOfTries` and `Settings.SMTPMinutesBetweenTry` from the legacy `smtpnoofretries` and `smtpminutesbetweenretries` `hm_settings.settinginteger` rows. Preserve the installed vtable/DISPIDs and legacy row-name mapping; do not read the obsolete `smtpnooftries` row, and keep setters, delivery retry scheduling changes, and broader Settings/Admin behavior out of scope.
