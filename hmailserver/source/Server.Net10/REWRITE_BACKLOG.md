...
6. COM/API compatibility.
   ...
   - Done: preserve the legacy `GroupMembers` and `GroupMember` COM vtable/identity contracts, register their hosted class identities, expose authenticated `Group -> Members` count/index/id lookup and read-only ID/group-ID/account-ID scalars from existing `hm_group_members` SQL data filtered by group and ordered by member ID, keep direct activation access-denied, and leave the Account child facade, ACL runtime behavior, and mutations explicit `E_NOTIMPL`.
...
Close the source queue-file lifecycle after successful delivery completion. Extend the existing delivery content-store boundary with a contained best-effort delete operation and invoke it only after lease-owned database completion succeeds for normal delivery, event-driven drop, and no-target completion. Never delete on defer, release, load failure, cancellation, or failed completion; preserve path-containment checks and leave orphan sweeping plus `DeliveryQueue.Clear`/`Remove` outside this slice.
