...
Continue the Administrator object model with a bounded read-only `Application -> Rules` global-rule slice: reuse the preserved legacy `Rules`/`Rule` COM contracts and existing store, expose authenticated global rule count/index/id lookup plus the same read-only core rule scalars from `hm_rules` rows with `ruleaccountid = 0`, and keep criteria, actions, execution, mutations, and deeper behavior explicit `E_NOTIMPL`.
