...
Continue legacy Administrator parity with the next bounded public-folder disk-name Settings slice: expose read-only `Settings.PublicFolderDiskName` as the legacy constant `#Public`. Preserve installed getter-only DISPID 79 and `BSTR` marshaling; keep filesystem access, public-folder creation/rename behavior, data-directory reconfiguration, and broader Settings/Admin mutation out of scope.
