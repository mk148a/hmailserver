...
6. COM/API compatibility.
   ...
   - Done: replace the `Scripting` placeholder with the complete legacy IID/vtable/class identity and hosted registration, expose authenticated read-only `Enabled`/`Language` from existing `usescriptserver`/`scriptlanguage` rows plus INI-backed `Directory`, preserve direct-activation access denial, and leave setters, reload, syntax compilation, current-file resolution, execution, and runtime reconfiguration explicit `E_NOTIMPL`.
...
Continue the bounded `Settings -> Scripting` parity with authenticated read-only `CurrentScriptFile`: derive the legacy path from the configured event directory and stored script language (`EventHandlers.vbs` for `VBScript`, `.js` for `JScript`, and an empty extension for unknown values) without reading the file. Preserve DISPID 6/`BSTR`, class identity, and direct-activation denial; keep setters, `Reload`, `CheckSyntax`, script execution, runtime reconfiguration, and broader Settings/Admin mutation out of scope.
