...
Continue the Administrator object model with a bounded read-only `Settings -> PublicFolders` root slice: reuse the preserved legacy `IMAPFolders`/`IMAPFolder` COM contracts, expose authenticated public-root folder count/index/name/id lookup plus the same read-only core folder scalars from `hm_imapfolders` rows with `folderaccountid = 0`, and keep messages, permissions, nested subfolders, mutations, and deeper behavior explicit `E_NOTIMPL`.
