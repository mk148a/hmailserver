...
Continue legacy Administrator parity with a bounded `Settings -> Scripting` slice: replace the placeholder with the complete legacy `Scripting` IID/vtable/class identity and hosted registration, then expose authenticated read-only `Enabled`/`Language` from existing `hm_settings` rows plus `Directory` from the configured/default legacy INI event directory. Keep setters, `Reload`, `CheckSyntax`, `CurrentScriptFile`, script compilation/execution changes, runtime reconfiguration, and broader Settings/Admin mutation out of scope.
