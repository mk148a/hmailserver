...
Continue legacy Administrator parity with the next bounded Settings numeric runtime slice: expose read-only `Settings.RuleLoopLimit`, `WorkerThreadPriority`, `TCPIPThreads`, and `MaxNumberOfMXHosts` from the existing `rulelooplimit`, `workerthreadpriority`, `tcpipthreads`, and `MaxNumberOfMXHosts` `hm_settings.settinginteger` rows. Preserve installed vtable/DISPIDs 48/57/60/90 and integer marshaling; keep setters, live rule loop/runtime thread tuning, SMTP MX scheduling behavior, and broader Settings/Admin mutation out of scope. Verify `MaxAsynchronousThreads` storage separately before including it in a later slice.
