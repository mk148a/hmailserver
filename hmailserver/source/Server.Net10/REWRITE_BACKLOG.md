...
5. Security and anti-abuse modernization.
   ...
   - Remaining: DKIM Administrator/COM setting wiring, packaged PSL snapshot/update/freshness lifecycle, and DMARC enforcement/Admin policy wiring; later Administrator/COM setting parity for SPF/greylisting knobs.
...
Continue DMARC operations with the next smallest safe slice: package a pinned official public-suffix snapshot into publish/install output and expose a deterministic refresh/freshness procedure, without introducing SMTP-path downloads or changing disabled-by-default/fail-open/no reject/quarantine behavior.
