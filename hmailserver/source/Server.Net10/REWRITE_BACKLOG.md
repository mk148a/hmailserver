...
Continue backup parity with a bounded `BackupManager.LoadBackup -> Backup` metadata slice: preserve and host the complete legacy `Backup` IID/vtable/class identity, read only the named `hMailServerBackup.xml` entry from a selected backup archive, map `BackupInformation Mode` bits 1/2/4 to `ContainsSettings`/`ContainsDomains`/`ContainsMessages`, and keep restore selections process-local. Preserve direct-activation denial; keep `StartBackup`, `StartRestore`, database/data-directory writes, archive extraction, restore execution, and backup events out of scope.
