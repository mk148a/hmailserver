...
- Done: Windows-only process-isolated SMTP rule script executor runs `EventHandlers.vbs`/`.js` through `cscript.exe`, timeboxes execution, round-trips message file mutations, exposes a file-backed scalar message facade, and fails closed when the script runner does not return status.
...
3. SMTP production pipeline.
   ...
   - Done: Windows-only process-isolated VBScript/JScript host for SMTP rule functions with file-backed scalar message facade (`FileName`, `DropMessage`, `RejectReason`, headers, common envelope fields, body fields, and `Save`).
   - Remaining: full legacy script object model, attachments, recipient collections, and event scripting hooks.
...
Implement the remaining legacy script object model behind the SMTP rule script executor, starting with attachment and recipient collection facades, then add protocol event scripting hooks.
