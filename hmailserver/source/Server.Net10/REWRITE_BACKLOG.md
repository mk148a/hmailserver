...
Continue legacy Administrator parity with a bounded `Application -> BackupManager` contract slice: replace the placeholder with the complete legacy IID/vtable/class identity and hosted registration, expose the authenticated child object through `Application.BackupManager`, and preserve direct-activation denial. Keep `StartBackup`, `LoadBackup`, backup parsing/execution, filesystem access, restore behavior, and event dispatch explicit `E_NOTIMPL` until the engine boundary lands separately.
