...
Continue mail-authentication with the next smallest safe DMARC slice: add an evaluation-only DMARC record/result foundation behind injected DNS, consuming SPF/DKIM outcomes only as data and avoiding SMTP reject/quarantine policy, signing, and Administrator/COM setting plumbing.
