...
Continue DMARC with the next smallest safe integration slice: add an injected organizational-domain/public-suffix boundary for SMTP DMARC evaluation so parent-domain records can be used without hard-coding suffix rules, while preserving disabled-by-default/fail-open/no reject/quarantine behavior and keeping Administrator/COM setting plumbing out of the slice.
