...
Continue legacy Administrator parity with the next bounded SMTP delivery connection-security Settings slice: expose read-only `Settings.SMTPConnectionSecurity` from existing `SmtpDeliveryConnectionSecurity` `hm_settings.settinginteger` data. Preserve installed DISPID 92 and `ComConnectionSecurity` enum values; keep the setter, WebAdmin/Admin mutation, live SMTP delivery routing behavior, delivery-worker reconfiguration, TLS policy changes, and broader Settings/Admin mutation out of scope.
