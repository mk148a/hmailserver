...
Continue legacy Administrator parity with the next bounded SMTP routing string slice: expose read-only `Settings.MirrorEMailAddress`, `DefaultDomain`, and `SMTPDeliveryBindToIP` from the existing `mirroremailaddress`, `defaultdomain`, and `smtpdeliverybindtoip` `hm_settings.settingstring` rows. Preserve installed vtable/DISPIDs 7/50/51 and `BSTR` marshaling; keep setters, relayer credentials, live SMTP routing/bind changes, and broader Settings/Admin behavior out of scope.
