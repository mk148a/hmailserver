...
Continue legacy Administrator parity with the next bounded Logging object slice: preserve the complete legacy `Logging` IID/vtable/class identity, register and host the class, and expose authenticated `Settings -> Logging` with read-only persisted logging flags/device/format from existing `hm_settings` rows. Keep direct activation access-denied and leave setters, live logging, current-log paths/file access, logger reconfiguration, and broader Settings/Admin mutation out of scope.
