...
Make durable outbound delivery operational with a bounded hosted-worker slice: add a single service-owned background loop that runs the existing `DeliveryQueueProcessor` with its configured options, waits without busy-spinning when no work is available, drains full batches promptly, and accepts an in-process wake signal from authenticated `DeliveryQueue.StartDelivery`. Preserve single-worker lease ownership, cancellation/shutdown behavior, direct-activation denial, and leave queue clear/remove plus SQL mutation outside this slice.
