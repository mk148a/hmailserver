...
6. COM/API compatibility.
   ...
   - Done: expose read-only non-secret SMTP relayer getters (`Settings.SMTPRelayer`, `SMTPRelayerRequiresAuthentication`, `SMTPRelayerUsername`, `SMTPRelayerPort`, and `SMTPRelayerConnectionSecurity`) from existing relayer rows, preserving DISPIDs 22/34/35/37/91 and marshaling while omitting `smtprelayerpassword`, leaving `SetSMTPRelayerPassword`, legacy `SMTPRelayerUseSSL`, setters, credential handling, and live SMTP delivery routing out of scope.
...
Continue legacy Administrator parity with the next bounded compatibility getter slice: expose authenticated read-only `Settings.SMTPRelayerUseSSL` from the already-loaded `SmtpRelayerConnectionSecurity` value, returning true only for legacy SSL/TLS (`ComConnectionSecurity.Tls`) and false for none or either STARTTLS mode. Preserve DISPID 71 and `VARIANT_BOOL` metadata; keep the setter, SQL changes, credential handling, live relay routing, and broader Settings/Admin mutation out of scope.
