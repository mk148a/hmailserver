...
the owning domain. Legacy `PasswordValidator::ValidatePassword` at
`hmailserver/source/Server/Common/Util/PasswordValidator.cpp:40-75` rejects
inactive accounts and inactive domains before password validation. Net10
`SqlServerAccountPasswordVerifier.AccountPasswordLookupSql` now joins
`hm_domains` and requires `accountactive <> 0` and `domainactive <> 0`; the
isolated fixture covers an active account attached to an inactive domain.

Focused verifier tests pass `3`, skip `1` because the disposable SQL target is
not configured; full Debug Net10 passes `2709`, skips `95`, and fails `0`.
Script override ordering and AD/SSPI verification remain separate gaps.
Release remains **RED**. Next slice: non-secret script override ordering and
failure containment, or atomic backup quiescence review.

## Current authoritative status (2026-08-25, inactive account password denial)

Code/test commit `d21480b09` closes the legacy inactive-account branch in
`SqlServerAccountPasswordVerifier.Verify`. Legacy
`InterfaceAccount::ValidatePassword` at
`hmailserver/source/Server/COM/InterfaceAccount.cpp:350-364` delegates to
`PasswordValidator::ValidatePassword`, which rejects inactive accounts before
password verification. Net10 now reads `accountactive` and fails closed before
script, AD, or legacy hash validation. The installed `Account` IID/vtable/
DISPID shape and authenticated ownership boundary are unchanged.

Focused verifier tests pass `3`, skip `1` for the unavailable disposable SQL
connection; full Debug Net10 passes `2709`, skips `95`, and fails `0`. Domain
active, AD, and script-backed validation remain separate gaps. Release remains
**RED**. Next slice: domain-active validation with an isolated SQL/store
contract, or the atomic backup quiescence design review if that environment is
not available.
...
The `Application.ServerState` lifecycle transition bridge is complete in
code/test commit `20e8f3d18`; it maps legacy `Stopped=1`, `Starting=2`,
`Running=3`, and `Stopping=4` across bootstrap/readiness/failure and host
shutdown paths without changing the COM contract. The next code slice is
isolated registered COM/SEC-18 caller evidence; it remains environment-gated.
...
passes `2652`, skips `94`, fails `0` (`2746` total). Release remains
**RED**. Open gates are Full-Text SQL/Data round-trip, target-identity native
filesystem review, private binding handle-based TOCTOU protection,
registered/out-of-process COM and SEC-18 caller evidence,
...
Net10 holds the generation-bound authorization lease around
`UpdateAddDeliveredToHeaderAsync`, fails closed when the lease is unavailable,
disposes it on success/failure, and publishes the retained snapshot only after
a successful update. Focused tests pass `4`, skip `0`, and fail `0`; full Net10
passes `2593`, skips `90`, and fails `0` (`2683` total). Settings
IID/vtable/DISPID/class identity, SQL schema, SMTP header behavior, and live
reconfiguration are unchanged.

Next independent slice: audit the next remaining authenticated Settings/Admin
mutation lease gap, prioritizing `MaxNumberOfMXHosts` or
`VerifyRemoteSslCertificate`. The disposable SQL/Data acceptance gate remains
blocked by the absent approved SQL connection and isolated-create opt-in.
Release remains **RED**.

## Historical bounded slice: DefaultDomain authorization lease (2026-08-21)
...
and persists the existing `hm_settings.defaultdomain` row seeded by
`hmailserver/source/DBScripts/CreateTablesMSSQL.sql:820`.

Net10 holds the generation-bound authorization lease around
`UpdateDefaultDomainAsync`, fails closed when the lease is unavailable,
disposes it on success/failure, and publishes the retained snapshot only after
a successful update. Focused tests pass `4`, skip `0`, and fail `0`; full Net10
passes `2591`, skips `90`, and fails `0` (`2681` total). Settings
IID/vtable/DISPID/class identity, SQL schema, default-domain runtime, and live
reconfiguration are unchanged.

Next independent slice: audit the next remaining authenticated Settings/Admin
mutation lease gap, prioritizing `AddDeliveredToHeader` or
`MaxNumberOfMXHosts`. The disposable SQL/Data acceptance gate remains blocked
by the absent approved SQL connection and isolated-create opt-in. Release
remains **RED**.

## Historical bounded slice: RuleLoopLimit authorization lease (2026-08-21)
...
Net10 holds the generation-bound authorization lease around
`UpdateRuleLoopLimitAsync`, fails closed when the lease is unavailable,
disposes it on success/failure, and publishes the retained snapshot only after
a successful update. Focused tests pass `5`, skip `0`, and fail `0`; full Net10
passes `2589`, skips `90`, and fails `0` (`2679` total). Settings
IID/vtable/DISPID/class identity, SQL schema, rule runtime, and live
reconfiguration are unchanged.

Next independent slice: audit the next remaining authenticated Settings/Admin
mutation lease gap. The disposable SQL/Data acceptance gate remains blocked by
the absent approved SQL connection and isolated-create opt-in. Release remains
**RED**.

## Historical bounded slice: MaxMessageSize authorization lease (2026-08-21)
...
Net10 holds the generation-bound authorization lease around
`UpdateMaxMessageSizeAsync`, fails closed when the lease is unavailable,
disposes it on success/failure, and publishes the retained snapshot only after
a successful update. Focused tests pass `4`, skip `0`, and fail `0`; full Net10
passes `2587`, skips `90`, and fails `0` (`2677` total). Settings
IID/vtable/DISPID/class identity, SQL schema, SMTP/IMAP message-size runtime,
and live reconfiguration are unchanged.
...
## Historical bounded slice: MaxAsynchronousThreads authorization lease (2026-08-21)
...
Net10 holds the generation-bound authorization lease around
`UpdateMaxImapConnectionsAsync`, fails closed when the lease is unavailable,
disposes it on success/failure, and publishes the retained snapshot only after
a successful update. Focused tests pass `4`, skip `0`, and fail `0`; full Net10
passes `2583`, skips `90`, and fails `0` (`2673` total). Settings
IID/vtable/DISPID/class identity, SQL schema, IMAP runtime behavior, and live
reconfiguration are unchanged.

Next independent slice: audit and close one remaining authenticated Settings
mutation lease gap, beginning with `MaxAsynchronousThreads` or
`MaxMessageSize`. The disposable SQL/Data acceptance gate remains blocked by
the absent approved SQL connection and isolated-create opt-in. Release remains
**RED**.

## Historical bounded slice: MaxDeliveryThreads authorization lease (2026-08-21)
...
`hmailserver/source/Server/COM/InterfaceBackupSettings.cpp:189-220`; it calls
`Configuration::SetBackupOption` at
`hmailserver/source/Server/Common/Application/Configuration.cpp:450-472`.
The flag is `HM::Backup::BOCompression = 8` in
...
only bit `8`, preserves all other bits, keeps the authenticated
Settings/lease boundary, and publishes snapshots only after one row succeeds.
The parent callback merges bit `8` against current parent options for retained
child safety.

Focused tests pass `282/282`; full Net10 passes `2569`, skips `90`, and fails
`0` (`2659` total). The next independent code slice is raw non-DB-only
`BODomains|BOMessages` DataBackup staging, leaving the external `DataBackup`
directory beside the archive. Disposable `6000` SQL/Data host-start,
restore/rollback, SEC-18, registered COM, paired C++ performance, and
long-soak gates remain environment-blocked or unproven. Release remains
**RED**.

## Historical bounded slice: BackupSettings message-flag persistence (2026-08-21)
...
`IInterfaceBackupSettings.BackupDomains` setter parity. Legacy
`InterfaceBackupSettings::get/put_BackupDomains` is at
`hmailserver/source/Server/COM/InterfaceBackupSettings.cpp:120-152`; it calls
`Configuration::SetBackupOption` at
`hmailserver/source/Server/Common/Application/Configuration.cpp:450-474`.
The flag is `HM::Backup::BODomains = 2` in
`hmailserver/source/Server/Common/Application/Backup.h:14-20`.
...
`hMailServer.BackupSettings.1`, and the existing vtable order. Net10 uses a
parameterized current-row update of `hm_settings.backupoptions` that changes
only bit `2`, preserves all other bits, keeps the authenticated owning
Settings/lease boundary, and publishes snapshots only after one row succeeds.

Focused tests pass `272/272`; full Net10 passes `2559`, skips `90`, and fails
`0` (`2649` total). The next independent code slice is `BackupMessages` bit
`4` parity. Disposable `6000` SQL/Data host-start, restore/rollback,
SEC-18, registered COM, paired C++ performance, and long-soak gates remain
...
`Settings.Backup.Destination` persistence parity. Legacy
`InterfaceBackupSettings::get_Destination/put_Destination` is in
`hmailserver/source/Server/COM/InterfaceBackupSettings.cpp`; the shared
configuration path is `Configuration::GetBackupDestination` and
`Configuration::SetBackupDestination` in
`hmailserver/source/Server/Common/Application/Configuration.cpp`, writing the
`hm_settings` row named `backupdestination`. The installed child identity and
property contract remain `IInterfaceBackupSettings` IID
`2C5559F0-DF3F-43C0-935C-F79D41CF8A5B`, Destination DISPID `1`, CLSID
`E0213ECF-BAEC-4E20-9813-0F75A97D0B16`, and ProgID
`hMailServer.BackupSettings.1`.

Net10 uses a parameterized SQL update with the schema width `nvarchar(4000)`
from `hmailserver/source/DBScripts/CreateTablesMSSQL.sql:299-303`, requires
the authenticated Administrator lease, and updates the COM snapshot only
after exactly one row succeeds. Administrator `ucBackup.SaveData` writes the
destination after the four option setters, preventing a destination-only
partial save while those setters remain pending. Focused tests pass `261/261`;
full Net10 passes `2548`, skips `90`, and fails `0` (`2638` total).

Residual scope: the four sibling BackupSettings option setters and complete
backup creation behavior remain incomplete. The next independent production
gate is disposable `6000` SQL/Data host-start and no-side-effect evidence;
approved SQL opt-in is still absent. Release remains **RED**.
...
(`hmailserver/source/Server/COM/InterfaceSettings.cpp:2393-2406`) calls
`Configuration::SetSslVersionEnabled`
(`hmailserver/source/Server/Common/Application/Configuration.cpp:632-640`),
which changes only bit `16` in the existing `hm_settings.SslVersions` integer
row. Code/test commit `0f974cdd9` adds the authenticated Net10 bitmask
...
`248`, fail `0`; full Net10 passes `2527`, skips `90`, fails `0`.
...
## Current bounded slice: authenticated TLS 1.2 flag persistence (2026-08-21)

Legacy `InterfaceSettings::put_TlsVersion12Enabled`
(`hmailserver/source/Server/COM/InterfaceSettings.cpp:2359-2372`) calls
`Configuration::SetSslVersionEnabled`
(`hmailserver/source/Server/Common/Application/Configuration.cpp:632-640`),
which changes only bit `8` in the existing `hm_settings.SslVersions` integer
row. Code/test commit `01fa372ff` adds the authenticated Net10 bitmask
mutation, unrelated-bit preservation, fail-closed outcome, and
publish-after-success snapshot behavior. Focused Settings/SQL tests pass
`247`, fail `0`; full Net10 passes `2526`, skips `90`, fails `0`.
...
which persists `hm_settings.LogonAttemptsWithinMinutes`, seeded by
`CreateTablesMSSQL.sql:912`. Code/test commit `cab54911a` adds the
authenticated Net10 parameterized integer mutation, one-row failure
containment, authorization lease, and publish-after-success snapshot behavior.
Focused Settings/SQL tests pass `240`, fail `0`.

The full suite reached `2516 passed, 90 skipped, 2 failed` (`2608` total); both
failures were unrelated ClamWin/custom scanner tests denied access to temporary
`.eml` files by host endpoint protection. No logon-failure algorithm, SMTP
trust, live reconfiguration, COM identity, or direct activation boundary
changed. The next slice remains disposable `6000` SQL/Data host-start
acceptance; release remains RED.
...
`hm_settings.settinginteger` value for `enableimapacl` through the configured
administration store, publishes the snapshot only after a one-row success, and
retains direct activation and failed-save behavior. Focused COM/SQL coverage
is `218 passed, 0 skipped, 0 failed`; the disposable LocalDB SQL integration
passed; full Net10 Debug is `2533 passed, 10 skipped, 0 failed` (`2543` total).

Legacy anchors are `InterfaceSettings::get/put_IMAPACLEnabled`
(`hmailserver/source/Server/COM/InterfaceSettings.cpp:1463-1490`) and
`IMAPConfiguration::Get/SetUseIMAPACL`
(`hmailserver/source/Server/IMAP/IMAPConfiguration.cpp:90-98`), with
`PROPERTY_ENABLEIMAPACL` and the existing `hm_settings` seed. The installed
Settings IID/vtable/DISPID `75` shape and authenticated server-administrator
boundary remain unchanged. This persistence-only slice does not change ACL
rights enforcement or live capability reconfiguration.

The next bounded slice is authenticated `Settings.IMAPSASLPlainEnabled`
persistence parity against `enableimapsaslplain`, with security review of the
authentication/runtime boundary. Do not add live IMAP capability changes,
credential behavior, or unrelated Admin mutations. Production-hosted
...
## Historical current slice (2026-08-21, IMAP ACL persistence security slice after IMAP IDLE parity)

Code/test commit `e27385413` closes the authenticated
`Settings.IMAPIdleEnabled` persistence gap. The setter updates the existing
`hm_settings.settinginteger` value for `enableimapidle` through the configured
administration store, publishes the snapshot only after a one-row success, and
retains direct activation and failed-save behavior. Focused COM/SQL coverage
is `216 passed, 0 skipped, 0 failed`; the disposable LocalDB SQL integration
passed; full Net10 Debug is `2530 passed, 10 skipped, 0 failed` (`2540` total).

Legacy anchors are `InterfaceSettings::get/put_IMAPIdleEnabled`
(`hmailserver/source/Server/COM/InterfaceSettings.cpp:1432-1458`) and
`IMAPConfiguration::Get/SetUseIMAPIdle`
(`hmailserver/source/Server/IMAP/IMAPConfiguration.cpp:78-86`), with
`PROPERTY_ENABLEIMAPIDLE` and the existing `hm_settings` seed. The installed
Settings IID/vtable/DISPID shape and authenticated server-administrator
boundary remain unchanged. IMAP IDLE capability/runtime reload is deliberately
out of scope.

The next bounded slice is authenticated `Settings.IMAPACLEnabled` persistence
parity against `enableimapacl`, but it requires a security review of the ACL
runtime and authorization consequences before implementation. Do not add live
ACL capability reconfiguration or unrelated Admin mutations. Production-hosted
SMTP/POP3 timing, migration/installer, SEC-18, paired C++ performance, and
soak remain separate gates. Release remains **RED**. No push was performed.

## Historical current slice (2026-08-21, IMAP IDLE persistence after IMAP QUOTA parity)
...
value for `protocolimap` through the configured administration store, publishes
the snapshot only after a one-row success, and retains direct activation and
failed-save behavior. Focused COM/SQL coverage is `208 passed, 0 skipped, 0
failed`; the disposable LocalDB SQL integration passed; full Net10 Debug is
`2518 passed, 10 skipped, 0 failed` (`2528` total).

Legacy anchors are `InterfaceSettings::get/put_ServiceIMAP`
(`hmailserver/source/Server/COM/InterfaceSettings.cpp:862-890`) and
`Configuration::GetUseIMAP/SetUseIMAP`
(`hmailserver/source/Server/Common/Application/Configuration.cpp:200-208`).
The installed Settings IID/vtable/DISPID shape and authenticated
server-administrator boundary remain unchanged. Live IMAP listener
reconfiguration is deliberately out of scope.

The next bounded slice is production-hosted SMTP enable/disable/timing
acceptance against the persisted protocol settings on an isolated
production-like host. It must prove socket behavior without changing
production state. Do not add live IMAP/POP3 listener reconfiguration, DKIM,
DNS verification, or unrelated Admin mutations. Migration/installer, SEC-18,
paired C++ performance, and soak remain separate gates. Release remains
**RED**. No push was performed.

## Historical current slice (2026-08-21, IMAP service persistence after ServicePOP3 parity)
...
(`hmailserver/source/Server/COM/InterfaceDomain.cpp:1008-1044`), with the
corresponding `Domain` setters in `source/Server/Common/BO/Domain.cpp`.
The COM test proves staged setter/save publication and failure retention; the
SQL integration proves `domainmaxsize` and `domainmaxaccountsize` update/readback
in the disposable store.
...
and member names/order, rejects duplicate group names, missing names, repeated
containers, and unexpected children. Focused coverage is `21 passed, 0 skipped,
0 failed`; the disposable LocalDB/Data full suite is `2439 passed, 10 skipped,
0 failed` (`2449` total).
...
`hmailserver/source/Server/Common/BO/GroupMembers.cpp:57-84`. This slice is
parser/model-only: it does not insert groups or members, resolve ACL holders,
or change restore transaction behavior. Next: transaction-scoped group/member
restore with restored-ID holder resolution and mid-batch rollback. Release
remains **RED**.

## Historical current slice (2026-08-20, group/member backup capture parity)
...
The latest parity audit also confirms the implemented `BlockedAttachments`
`Add`/`Save`/`DeleteByDBID`/item `Delete` mutation path against the legacy COM
sources; focused `BlockedAttachmentsComContractTests` coverage is `15/15`.
...
`SMTPConfiguration::SetSMTPConnectionSecurity`
(`source/Server/SMTP/SMTPConfiguration.cpp:175-184`), which persists
`PROPERTY_SMTPCONNECTIONSECURITY` (`Constants.h:121`) in the existing
`SmtpDeliveryConnectionSecurity` row seeded by `CreateTablesMSSQL.sql:934`.
The legacy setter performs no enum validation and returns S_OK after the
delegated persistence path. Net10 now uses a parameterized fixed-row update,
requires exactly one affected row, rechecks the server-admin boundary, retains
the prior snapshot on failure, and publishes the snapshot only after success.

Focused Settings/SQL coverage is `142/142`; full Net10 is `2133 passed, 53
skipped, 0 failed`. Installed COM identity, direct activation, SMTP trust, and
live runtime reconfiguration remain unchanged. Next independent slice: fresh
legacy-first audit of one remaining fixed-row Settings mutation. The
...
## Historical current next slice (superseded, 2026-08-11, MaxAsynchronousThreads admin mutation)
...
three loopback `hm_tcpipports` rows, and message filenames below the selected
Data root. It also records whether every accepted message became a new queued
or delivered row. Legacy references are
`SMTPConnection::HandleSMTPFinalizationTaskCompleted_` (completion at
...
The Net10 diagnostic accepted `1/1` and observed one new queued row, but the
exact fixture gate is **FAIL** because the current database has no matching
Inbox row and all 1028 pre-run message filenames are outside the selected Data
root; the post-run outside count is 1029. C++ remains blocked before launch by
the legacy Registry32 configuration boundary. The performance release gate is
**RED**. Next smallest slice: recreate fresh equal disposable SQL/Data/message
roots, prove filename and Inbox identity, then verify SQL Full-Text Search.
...
acquired immediately before the existing parameterized
`smtpallowincorrectlineendings` SQL update and held through result handling
and retained snapshot publication. SMTP bare-LF runtime behavior remains out
of scope for this bounded mutation slice.

Legacy behavior is anchored by
`InterfaceSettings::get/put_AllowIncorrectLineEndings`
...
`smtpallowincorrectlineendings` seed (`source/DBScripts/CreateTablesMSSQL.sql:842`).
The .NET `UpdateAllowIncorrectLineEndingsSql` shape was not changed. Focused
coverage is `125/125`, including unavailable-lease denial and in-flight
reauthentication blocking.

Legacy SMTP bare-LF validation consumes this setting
(`source/Server/SMTP/SMTPConnection.cpp:1259-1265`); that runtime path is
unchanged. Full unfiltered Net10 is `2108 passed, 39 skipped, 0 failed`.
Release remains **RED** for disposable SQL/Data restore, non-DB restore,
...
legacy-first audit of `Settings.MaxSMTPRecipientsInBatch`.
...
parameterized `smtprelayerusername` SQL update and held through result handling
and retained snapshot publication. SMTP credential handling and live
reconfiguration remain out of scope.

Legacy behavior is anchored by `InterfaceSettings::get/put_SMTPRelayerUsername`
...
`smtprelayerusername` seed (`source/DBScripts/CreateTablesMSSQL.sql:782`).
The .NET `UpdateSmtpRelayerUsernameSql` shape was not changed. Focused
coverage is `104/104`.

The unfiltered full run has 2 unrelated host/AV cleanup failures in the
scanner test classes (`UnauthorizedAccessException` deleting temporary `.eml`
files); excluding those classes, full Net10 is `2080 passed, 39 skipped, 0
failed`. Remaining unleased Settings/COM/Admin mutations and the disposable
SQL/Data restore, non-DB restore, SQL/FTS, paired performance, SEC-18,
migration/installer, out-of-process COM, AD/DC, crash/power-loss, and soak
gates remain RED. Next slice: fresh legacy-first audit of `SMTPRelayerPort`.

## Current Audit Note (2026-08-11, SMTP RELAYER AUTHENTICATION AUTHORIZATION LEASE)
...
`usesmtprelayerauthentication` SQL update and held through result handling and
retained snapshot publication. SMTP delivery authentication and live
reconfiguration remain out of scope.

Legacy behavior is anchored by
`InterfaceSettings::get/put_SMTPRelayerRequiresAuthentication`
...
`usesmtprelayerauthentication` seed (`source/DBScripts/CreateTablesMSSQL.sql:786`).
The .NET `UpdateSmtpRelayerRequiresAuthenticationSql` shape was not changed.
Focused coverage is `102/102`.

The unfiltered full run has 2 unrelated host/AV cleanup failures in the
scanner test classes (`UnauthorizedAccessException` deleting temporary `.eml`
files); excluding those classes, full Net10 is `2078 passed, 39 skipped, 0
failed`. Remaining unleased Settings/COM/Admin mutations and the disposable
SQL/Data restore, non-DB restore, SQL/FTS, paired performance, SEC-18,
migration/installer, out-of-process COM, AD/DC, crash/power-loss, and soak
...
## Current Audit Note (2026-08-11, SMTP RELAYER AUTHORIZATION LEASE)
...
parameterized `smtprelayer` SQL update and held through result handling and
retained snapshot publication. SMTP delivery runtime and live reconfiguration
remain out of scope.

Legacy behavior is anchored by `InterfaceSettings::get/put_SMTPRelayer`
...
`smtprelayer` seed (`source/DBScripts/CreateTablesMSSQL.sql:760`). The .NET
`UpdateSmtpRelayerSql` shape was not changed. Focused coverage is `100/100`.

The unfiltered full run had 2 unrelated host/AV cleanup failures in the
scanner test classes (`UnauthorizedAccessException` deleting temporary `.eml`
files); excluding those classes, full Net10 is `2076 passed, 39 skipped, 0
failed`. Remaining unleased Settings/COM/Admin mutations and the disposable
SQL/Data restore, non-DB restore, SQL/FTS, paired performance, SEC-18,
migration/installer, out-of-process COM, AD/DC, crash/power-loss, and soak
gates remain RED. Next slice: fresh legacy-first audit of
...
parameterized `allowmailfromnull` SQL update and held through result handling
and retained snapshot publication. The legacy inverted boolean persistence is
unchanged; SMTP empty-sender enforcement and live reconfiguration remain out
of scope.

Legacy behavior is anchored by `InterfaceSettings::get/put_DenyMailFromNull`
...
`allowmailfromnull` seed (`source/DBScripts/CreateTablesMSSQL.sql:736`).
The .NET `UpdateAllowMailFromNullSql` shape was not changed. Focused coverage
is `94/94`; full Net10 is `2077 passed, 39 skipped, 0 failed`.

Remaining unleased Settings/COM/Admin mutations and the disposable SQL/Data
restore, non-DB restore, SQL/FTS, paired performance, SEC-18,
migration/installer, out-of-process COM, AD/DC, crash/power-loss, and soak
gates remain RED. Next slice: fresh legacy-first audit of `SMTPNoOfTries`.
...
Focused `DiagnosticsComContractTests` coverage is `7 passed, 0 failed, 0 skipped`; full Net10 is `1967 passed, 32 skipped, 2 failed`, with the two known host-AV `.eml` cleanup locks. The security review passed this bounded slice. Release reality remains RED: approved disposable SQL/Data restore and Message.Save integration, migration/installer rollback, SEC-18, service/out-of-process COM, live SQL/FTS/protocol/load, AD/DC, crash/power-loss, and 24-hour soak evidence remain open. The Diagnostics runtime is still a test-configured abstraction; actual legacy DNS/SMTP/filesystem diagnostic execution remains out of scope. No production resources or COM registration were changed.

Next independent work: run approved disposable SQL/Data restore and Message.Save integration when its connection/isolated-create opt-in exists; otherwise perform a fresh legacy-first audit of the smallest remaining COM/Admin gap. Re-run scanner cleanup only on an AV-compatible path and do not weaken cleanup assertions.

## Current Audit Note (2026-08-10, IMAP MESSAGE.SAVE STATE/UID AND MULTI-DRAFT PUBLICATION PARITY)

Code/test commit `c1b1734c0` completes the bounded Message.Save state/UID publication slice. Legacy `InterfaceMessages::Add` and `InterfaceMessage::Save` (`hmailserver/source/Server/COM/InterfaceMessages.cpp:102-138`; `InterfaceMessage.cpp:390-516`) create an ID-zero Created draft that is not added to the parent collection, then save exactly one Delivered row. `PersistentMessage::AddObject` (`PersistentMessage.cpp:542-666`) obtains one folder UID through `PersistentIMAPFolder::GetUniqueMessageID` (`PersistentIMAPFolder.cpp:236-247`) and writes the generated ID back to the object. The .NET `SqlServerMessageAdministrationStore` now allocates `foldercurrentuid` and inserts `hm_messages` in one transaction, returns `MessageAdministrationInsertResult`, enforces account/folder ownership on reads, and `Messages.cs` publishes a saved draft once using the live collection. Multiple unsaved drafts saved in either order retain both IDs and UIDs.

Focused Message/store coverage is `39 passed, 1 skipped, 0 failed`; the skipped SQL integration test requires `HMAILSERVER_NET10_SQLSERVER_INTEGRATION_CONNECTION` and `HMAILSERVER_NET10_SQLSERVER_INTEGRATION_ALLOW_ISOLATED_CREATE=1`. Full Net10 is `1965 passed, 32 skipped, 2 failed`; the two failures are host-AV `UnauthorizedAccessException` cleanup failures in `ClamWinScannerTestRuntimeTests` and `CustomScannerTestRuntimeTests`. Security is YELLOW for this bounded slice and reality is RED for release. Remaining risks include missing `.eml` MIME persistence for COM-created drafts, legacy/.NET cross-writer UID coordination, and unavailable disposable SQL acceptance. No SMTP trust, live reconfiguration, protocol APPEND, COM identity, direct activation, production SQL/Data, service, IIS, DCOM, or firewall behavior was broadened.

Next independent work: obtain an approved disposable SQL/Data target for executed restore/rollback and Message.Save integration evidence; otherwise continue with a fresh parity audit of the smallest remaining COM/Admin gap. Keep AV-compatible scanner cleanup and release-gate work environment-blocked rather than weakening test cleanup.

## Current Audit Note (2026-08-10, IMAP FOLDER MESSAGE OWNERSHIP AND STALE-HANDLE PARITY)

Code/test commit `e311058e8` closes the bounded empty-folder owner-ID and retained-deleted-folder insertion gap. Legacy `InterfaceIMAPFolder::get_Messages` and `InterfaceMessages::Add` (`hmailserver/source/Server/COM/InterfaceIMAPFolder.cpp:161-178`; `InterfaceMessages.cpp:102-130`) retain the owning account/folder IDs for empty folders. For a deleted non-INBOX folder, legacy `InterfaceMessage::SaveNewMessageToIMAPFolder_` (`InterfaceMessage.cpp:400-402`) fails because `PersistentMessage::AddObject` (`PersistentMessage.cpp:587-618`) cannot obtain a current UID from the missing folder row through `PersistentIMAPFolder::GetCurrentUID_` (`PersistentIMAPFolder.cpp:193-223`). The .NET `IMAPFolder.Messages` adapter (`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/IMAPFolders.cs:365-368`) now passes `ImapFolderAdministrationSnapshot.AccountId`; `SqlServerMessageAdministrationStore.InsertMessageSql` (`hmailserver/source/Server.Net10/src/HMailServer.Storage.SqlServer/SqlServerMessageAdministrationStore.cs:47-63`) performs an atomic folder/account-qualified insert with `UPDLOCK,HOLDLOCK`.

Focused message/store/IMAP coverage is `36 passed, 5 skipped, 0 failed`; full Net10 is `1962 passed, 32 skipped, 2 failed`, with the two known host-AV scanner `.eml` cleanup failures. The new disposable SQL retained-folder acceptance test is skipped because `HMAILSERVER_NET10_SQLSERVER_INTEGRATION_CONNECTION` and `HMAILSERVER_NET10_SQLSERVER_INTEGRATION_ALLOW_ISOLATED_CREATE=1` are unset. Security review is YELLOW for the bounded slice because legacy delivered-state/folder-UID publication remains a separate open Message.Save parity item; no authorization, COM identity, SQL injection, SMTP, or production-state regression was found. Reality remains RED for release.

The next independent code slice is parity-confirmed IMAP `Message.Save()` delivered-state and folder-UID publication; it must preserve the current COM identity and owner scope and remain separate from protocol APPEND. Other next gates remain approved populated SQL/Data restore acceptance and AV-compatible scanner cleanup. Do not use production SQL/Data or stage protected SEC-18/benchmark artifacts.
...
6. COM/API compatibility.
   ...
   - Done: expose authenticated `AntiVirus.TestCustomerScanner(command, returnCode)` through a narrow process/file runtime boundary, preserve installed DISPID 16, direct-activation denial, legacy success flag plus result-text shape, clean/EICAR test checks, `%FILE%` substitution, and SEC-04 quote/escaping of the generated test-file path while using shell-free structured process arguments. Antivirus settings, SMTP/external-fetch virus scanning behavior, live scanner reconfiguration, and broader custom-scanner configuration/policy remain unchanged.
    - Done: replace the `BlockedAttachments`/`BlockedAttachment` placeholders with complete legacy IID/vtable/class identities and hosted registration, expose authenticated `Settings -> AntiVirus -> BlockedAttachments` count/index/DBID lookup plus `ID`, `Wildcard`, and `Description` from existing `hm_blocked_attachments` rows ordered by wildcard, and preserve the legacy add/delete/save collection mutation boundary through the configured administration store.
    - Done: implement authenticated legacy `BlockedAttachments.Refresh` collection reload through the configured blocked-attachment administration store, atomically replacing only that authorized COM collection's process-local snapshot so `Count`, index/DBID lookup, and item getters observe current blocked-attachment rows. Direct activation remains access-denied; contained reload failure maps to COM `E_FAIL` and retains the prior snapshot; scanner test/runtime execution, attachment-policy runtime changes, and broader SMTP mutation remain out of scope.
    - Done: implement authenticated legacy `BlockedAttachments.Add`, `DeleteByDBID`, `BlockedAttachment.Save`, `BlockedAttachment.Delete`, and scalar setters with owner-scoped snapshots and parameterized `hm_blocked_attachments` insert/update/delete operations. Focused `BlockedAttachmentsComContractTests` coverage is `15 passed, 0 failed`; direct activation remains access-denied and scanner/SMTP policy behavior remains unchanged.
...
- Superseded by the authenticated mutation slice recorded above: the .NET BlockedAttachments adapter and SQL administration store now implement collection Add/DeleteByDBID, item setters/Save/Delete, and owner-scoped snapshot publication inside the authenticated Settings boundary. Direct child activation remains denied; SMTP attachment-policy behavior, live reconfiguration, service/database/Data-directory state, and SEC-18 staging state remain unchanged. Next slice: `background_route_save.php` POST-only/CSRF hardening.
...
Focused Rule/SQL-contract coverage is `30 passed, 0 failed, 0 skipped`; full Net10 is `1977 passed, 32 skipped, 2 failed`, with the two known host-AV `.eml` cleanup locks. Security review is conditional PASS after the retained-save regression fix; reality is YELLOW for this slice and RED for release. Live SQL readback/rollback/concurrency and out-of-process COM evidence are unavailable; broader RuleCriteria/RuleAction mutation remains out of scope. No production resources or COM registration changed.

Next independent work: provision the approved disposable SQL target for real Rule move readback, rollback, ownership, and concurrency acceptance; otherwise continue with the smallest legacy-first COM/Admin gap. Keep AV cleanup on an AV-compatible host and do not stage protected SEC-18/benchmark artifacts.

## Current Audit Note (2026-08-10, ACCOUNT.UNLOCKMAILBOX POP3 LOCK PARITY)

Code/test commit `f89890421` completes the bounded `Account.UnlockMailbox()` parity slice. Legacy `InterfaceAccount::UnlockMailbox` (`hmailserver/source/Server/COM/InterfaceAccount.cpp:332`) calls process-local `POP3Sessions::Unlock(accountId)` and returns `S_OK`; POP3 acquisition and disconnect release are in `POP3Connection.cpp:496,831-838`. The .NET implementation wires an account-ID callback through `AccountAdministrationRuntimeHost`, the service POP3 lock manager, authenticated Accounts, synthetic Administrator ID 0, and the `Links` fallback account. `InMemoryPop3MailboxLockManager` uses owner-matched leases so an old lease cannot remove a replacement after forced unlock. No IID/CLSID/ProgID/DISPID/vtable, direct activation, authenticated Settings boundary, SMTP trust, live reconfiguration, SQL schema, or unrelated Admin collection behavior changed.

Focused Account/Application/Links/POP3 coverage is `87 passed, 0 failed, 0 skipped`; full Net10 is `1972 passed, 32 skipped, 2 failed`. Both failures are host-AV `.eml` cleanup locks; AV-excluded full is `1967 passed, 32 skipped, 0 failed`. Security review approves this bounded slice. Reality review remains RED for release: disposable SQL/Data restore and round-trip rollback, SEC-18, migration/installer, service/out-of-process COM, live SQL/FTS/protocol load, AD/DC, and 24-hour soak evidence remain open. The operation remains process-local and intentionally does not alter SMTP trust or protocol behavior.

Next independent work: use the approved disposable SQL/Data target for restore and Message.Save acceptance when available; otherwise perform a fresh legacy-first audit of the smallest remaining COM/Admin gap. Keep scanner cleanup on an AV-compatible path and do not stage protected SEC-18/benchmark artifacts.
## Current Authoritative Continuation (2026-08-10, SQL Account.ValidatePassword and Disposable LocalDB)

Code/test commit `f34ee25c8` closes the bounded attached, authenticated SQL credential lookup slice for `Account.ValidatePassword`. Legacy anchors are `InterfaceAccount::ValidatePassword` (`hmailserver/source/Server/COM/InterfaceAccount.cpp:350-364`), `PasswordValidator::ValidatePassword` (`hmailserver/source/Server/Common/Util/PasswordValidator.cpp:109-188`), `Crypt::Validate` (`hmailserver/source/Server/Common/Util/Crypt.cpp:63-84`), `PersistentAccount::ReadObject` (`hmailserver/source/Server/Common/Persistence/PersistentAccount.cpp:146-192`), and `CreateTablesMSSQL.sql` (`hmailserver/source/DBScripts/CreateTablesMSSQL.sql:168-194`). Current symbols are `SqlServerAccountPasswordVerifier` (`hmailserver/source/Server.Net10/src/HMailServer.Storage.SqlServer/SqlServerAccountPasswordVerifier.cs`), `AccountAdministrationRuntimeHost.Configure`, and `AccountComClass.ValidatePassword`. The verifier uses a constant parameterized query by immutable account ID, preserves the legacy script-first/empty-password/AD/local-hash order through existing abstractions, and fails closed for missing, invalid, empty, and SQL-error rows. It intentionally does not add active-account, username, alias, last-logon, or auto-ban policy to the direct COM method.

The user-owned `MSSQLLocalDB` disposable environment is now available through `build/prepare-net10-disposable-localdb.ps1`; the paired removal script requires a marker-protected TEMP Data root and explicit GUID-scoped database names. `MSSQLSERVER` and `HmailDb_Test5700` were not used. Focused verifier/COM/password coverage passed `70/70`; the new SQL integration test passed `4/4` with TRX evidence in `artifacts/net10-disposable/`. Full Net10 passed `2009`, skipped `2`, and failed `9` existing restore/message-index fixture/schema tests. This is execution evidence, not release acceptance.

Security is CONDITIONAL because valid SQL credentials can bypass a configured legacy `OnClientValidatePassword` rejection and AD accounts are rejected rather than validated. Reality is RED: SEC-12 rate-limit/auto-ban behavior, SEC-18, populated restore/rollback, migration/installer, out-of-process COM, live SQL/protocol load, and 24-hour soak remain blockers. Next bounded slice is isolated SQL restore fixture/schema repair and rerun; keep COM script and AD verification separate and independently reviewed. Do not modify production SQL/Data, service, COM registration, DCOM, IIS, firewall, or hMailServer installation state.

## Current Authoritative Continuation (2026-08-10, ISOLATED RESTORE FIXTURE SCHEMA REPAIR)

Test-only code/test commit `877f72160` repairs the disposable SQL schema in `BackupRestoreRoundTripIntegrationTests.CreateTargetSchemaAsync`. The fixture now has the legacy `hm_fetchaccounts.faid` identity plus the minimal `hm_imapfolders`, `hm_acl`, `hm_group_members`, and `hm_fetchaccounts_uids` columns referenced by `SqlServerDomainAdministrationStore.DeleteAllDomainsForRestoreSql`. This fixes the observed `Invalid column name 'faid'` drift before injected alias/recipient failure points; no production code or installed state changed.

Legacy references: `BackupExecuter::StartRestore`/`RestoreDataDirectory_` (`hmailserver/source/Server/Common/Application/BackupExecuter.cpp:230-388`), `Collection<T,P>::XMLLoad/DeleteAll` (`hmailserver/source/Server/Common/BO/Collection.h:85-135,202-215`), and `CreateTablesMSSQL.sql` (`hmailserver/source/DBScripts/CreateTablesMSSQL.sql:355-370,433-469,627-648`). Current symbols: `SqlServerDomainAdministrationStore.DeleteAllDomainsForRestoreAsync`, `SqlServerDomainAdministrationStore.DeleteAllDomainsForRestoreSql`, and `MetadataBackupRestoreExecutor.RestoreMetadataAsync`.

The disposable LocalDB `BackupRestoreRoundTripIntegrationTests` class passes `11/11`; default full Net10 passes `1987`, skips `33`, and fails `0`; `git diff --check` passes. This slice does not claim full populated restore parity, crash-safe SQL/filesystem atomicity, normal-installation deletion/reinitialization, service/out-of-process COM, SEC-18, migration/installer, live C++/.NET 10 load, or soak acceptance. Release remains RED. Next bounded slice: extend isolated populated SQL restore graph readback/rollback from the repaired fixture without touching production resources or the protected dirty files.

## Current Authoritative Continuation (2026-08-10, FETCHACCOUNT RESTORE)

Code/test commit `7e8d71c15` closes the bounded restore-side FetchAccount/FetchAccountUID gap. Legacy `Account::XMLStore` (`hmailserver/source/Server/Common/BO/Account.cpp:280-331`) serializes FetchAccounts; `FetchAccount::XMLStore` (`hmailserver/source/Server/Common/BO/FetchAccount.cpp:55-79`) writes the encrypted password and nested UID container; `FetchAccountUID::XMLStore` (`hmailserver/source/Server/Common/BO/FetchAccountUID.cpp:42-49`) writes `UID` and `Date`; and `FetchAccounts::Refresh`/`FetchAccountUIDs::Refresh` (`hmailserver/source/Server/Common/BO/FetchAccounts.cpp:36-43`, `FetchAccountUIDs.cpp:29-50`) preserve owner scoping. The parser now reconstructs FetchAccount scalar fields, legacy `UseSSL` compatibility, encrypted password, and UID children. `BackupRestoreMetadataWriter.RestoreFetchAccountsAsync` inserts the generated parent ID and UID rows; malformed ciphertext/date fails closed.

The production path is transaction-scoped: `IBackupRestoreMetadataTransaction.FetchAccountStore` is supplied by `SqlServerBackupRestoreMetadataTransactionFactory`, and `SqlServerFetchAccountAdministrationStore` uses the shared `SqlServerBackupRestoreTransactionContext` for FetchAccount and UID inserts. Focused parser/SQL/restore coverage is `30 passed, 0 failed, 0 skipped`; disposable LocalDB readback plus transaction rollback is `2 passed, 0 failed, 0 skipped`; default full Net10 is `1990 passed, 35 skipped, 0 failed`. SQL-enabled full Net10 is `2017 passed, 2 skipped, 6 unrelated existing message/indexing fixture failures. No COM identity, authenticated access boundary, SMTP trust, production SQL/Data, service, IIS, DCOM, or machine state changed.

This slice does not claim FetchAccount COM saved-item mutation, full populated backup/restore graph, settings/public-folder/message restore, SQL/filesystem crash recovery, SEC-18, migration/installer, service/out-of-process COM, live paired C++/.NET 10 performance, AD/DC, protocol/load, or 24-hour soak acceptance. Release remains RED. Next slice: extend the disposable restore fixture with populated FetchAccount/readback and injected UID/date failure rollback at executor level, or move to the next independent protocol harness only when approved FTS/legacy ADO staging exists.
...
installer, paired protocol performance, and soak gates remain RED or
environment-blocked.
## Current Audit Note (2026-08-10, WELCOME IMAP ADMIN MUTATION)

Code/test commit `df7f72c22` closes the narrow authenticated
`IInterfaceSettings.WelcomeIMAP` setter gap (`DispId(25)`, BSTR) while
preserving the installed COM shape. Legacy behavior is anchored by
`IInterfaceSettings.WelcomeIMAP` (`DispId(25)`),
...
COM, IMAP, and common application sources.

The .NET setter requires the authenticated settings boundary and live server
administrator callback, updates only the existing `welcomeimap` row through a
parameterized SQL command, requires exactly one affected row, and publishes the
retained snapshot only after success. Direct activation, failed-write snapshot
retention, retained-object reauthentication, and SQL shape are covered.
Focused coverage is `35/35`; full Net10 is `2018 passed, 39 skipped, 0 failed`.

IMAP greeting/session runtime wiring and live reconfiguration remain out of
scope. The next code slice must be selected by a fresh legacy-first audit of
one remaining low-risk Settings setter. Real SQL/Data rollback, SEC-18,
installer, paired protocol performance, and soak gates remain RED or
environment-blocked.
...
`IInterfaceSettings.SMTPRelayerPort` setter (`DispId(37)`, `int`) only. The
.NET path updates the existing `hm_settings.smtprelayerport` row through a
parameterized `SqlDbType.Int` command, requires exactly one affected row,
rechecks the live server-administrator callback, and publishes the retained
snapshot only after success. The legacy value is written unchanged and the
seeded default remains `25`. Direct activation denial, authorized integer
write, failed-write retention, administrator revocation, one-row enforcement,
and exact SQL shape are covered. Focused settings/store coverage is `74/74`;
full Net10 is `2057 passed, 39 skipped, 0 failed`.

Legacy anchors are `IInterfaceSettings.SMTPRelayerPort`
(`source/Server/hMailServer/hMailServer.idl`),
`InterfaceSettings::get_SMTPRelayerPort` and
...
`PropertySet::SetLong` and `Property::WriteLongSetting_`,
`ServerTargetResolver::GetFixedSMTPHostForDomain_`, and the
`smtprelayerport` seed (`source/DBScripts/CreateTablesMSSQL.sql`). Fixed-
relayer delivery routing, value `0` fallback behavior, configuration
notifications, and live reconfiguration were deliberately not changed; this
is COM persistence parity only.
...
Focused permission coverage is `29 passed, 0 failed`; combined IMAP
folder/permission/SQL-store coverage is `63 passed, 0 failed`; full Net10 is
`2269 passed, 55 skipped, 0 failed`. Retained wrappers can still read or stage
in-memory values after logout; this is outside the persistence-mutation slice.
Release remains **RED** because approved SQL/Data readback, paired C++/.NET
...
Release remains **RED**.
## Current next slice (2026-08-14, protocol participant registration)

Code/test commit `0e9164404` completes the bounded protocol lifecycle slice.
`AddProductionHostedServices` now registers one
`ServiceReinitializationCoordinator`; IMAP, POP3, and SMTP hosted services
register owned restart callbacks in that order. Coordinator reinitialization
stops them in reverse order, starts them in registration order, keeps the
host-owned `ExecuteAsync` task alive during coordinator stop/start, and starts
a fresh readiness generation that is released only after successful restart.
Focused lifecycle/registration coverage is `25 passed, 0 failed`; full Net10
Debug is `2307 passed, 57 skipped, 0 failed`.
...
This does not implement SQL-driven `hm_tcpipports` discovery, the restore
callback, or `ApplicationComClass.Reinitialize`; no COM identity or SMTP
trust behavior changed. Restore/rollback, paired C++/.NET performance, SEC-18,
migration/installer, out-of-process COM, and soak gates remain open; release
remains **RED**.
...
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.ClamWinExecutable mutation (2026-08-22)

Code/test commit `aff52ba5d` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_ClamWinExecutable` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:70-85` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`avclamwinexec` setting, and returns `S_OK` on success. Net10 preserves the
existing Settings authentication boundary and COM identity, performs a
parameterized one-row update through `ISettingsAdministrationMutationStore`,
and updates the retained Settings snapshot only after persistence succeeds.

Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `321 passed, 0 skipped, 0
failed` focused and `2608 passed, 92 skipped, 0 failed` full Debug (`2700`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.

Next independent slices, in order: establish or record the exact blocker for
Full-Text-capable disposable SQL Server `6000`; implement the authenticated
`AntiVirus.ClamWinDBFolder` mutation; then continue isolated registered
COM/SEC-18 caller evidence. Do not broaden this slice to SMTP trust, live
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.ClamWinDBFolder mutation (2026-08-22)

Code/test commit `16ba1c809` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_ClamWinDBFolder` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:104-119` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`avclamwindb` setting, and returns `S_OK` on success. Net10 preserves the
existing Settings authentication boundary and COM identity, performs a
parameterized one-row update through `ISettingsAdministrationMutationStore`,
and updates the retained Settings snapshot only after persistence succeeds.

Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `324 passed, 0 skipped, 0
failed` focused and `2611 passed, 92 skipped, 0 failed` full Debug (`2703`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.Action mutation (2026-08-22)

Code/test commit `304796fd4` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_Action` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:248-276` returns access
denied before mutation for unauthenticated callers, maps the two installed
enum values to legacy integers `0` and `1`, writes the fixed `avaction` setting,
and returns `S_OK` on success. Net10 preserves the existing Settings
authentication boundary and COM identity, performs a parameterized one-row
update through `ISettingsAdministrationMutationStore`, and updates the
retained Settings snapshot only after persistence succeeds.

Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `327 passed, 0 skipped, 0
failed` focused and `2614 passed, 92 skipped, 0 failed` full Debug (`2706`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.NotifyReceiver mutation (2026-08-22)

Code/test commit `5e48295f7` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_NotifyReceiver` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:326-347` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`avnotifyreceiver` setting as integer `0` or `1`, and returns `S_OK` on
...
Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `330 passed, 0 skipped, 0
failed` focused and `2617 passed, 92 skipped, 0 failed` full Debug (`2709`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.NotifySender mutation (2026-08-22)

Code/test commit `1444bea7e` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_NotifySender` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:292-309` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`avnotifysender` setting as integer `0` or `1`, and returns `S_OK` on success.
Net10 preserves the existing Settings authentication boundary and COM
identity, performs a parameterized one-row update through
`ISettingsAdministrationMutationStore`, and updates the retained Settings
snapshot only after persistence succeeds.

Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `333 passed, 0 skipped, 0
failed` focused and `2620 passed, 92 skipped, 0 failed` full Debug (`2712`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.CustomScannerEnabled mutation (2026-08-22)

Code/test commit `40e77eb2b` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_CustomScannerEnabled` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:138-157` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`usecustomvirusscanner` setting as integer `0` or `1`, and returns `S_OK` on
success. Net10 preserves the existing Settings authentication boundary and
COM identity, performs a parameterized one-row update through
`ISettingsAdministrationMutationStore`, and updates the retained Settings
snapshot only after persistence succeeds.
...
Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `336 passed, 0 skipped, 0
failed` focused and `2623 passed, 92 skipped, 0 failed` full Debug (`2715`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.CustomScannerExecutable mutation (2026-08-22)

Code/test commit `f02091d5f` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_CustomScannerExecutable` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:172-189` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`customvirusscannerexecutable` setting, and returns `S_OK` on success. Net10
preserves the existing Settings authentication boundary and COM identity,
performs a parameterized one-row update through
`ISettingsAdministrationMutationStore`, and updates the retained Settings
snapshot only after persistence succeeds.

Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `339 passed, 0 skipped, 0
failed` focused and `2626 passed, 92 skipped, 0 failed` full Debug (`2718`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.CustomScannerReturnValue mutation (2026-08-22)

Code/test commit `3f66a8eb9` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_CustomScannerReturnValue` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:206-221` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`customviursscannerreturnvalue` setting, and returns `S_OK` on success. Net10
preserves the existing Settings authentication boundary and COM identity,
performs a parameterized one-row update through
`ISettingsAdministrationMutationStore`, and updates the retained Settings
snapshot only after persistence succeeds.
...
Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `342 passed, 0 skipped, 0
failed` focused and `2629 passed, 92 skipped, 0 failed` full Debug (`2721`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
reconfiguration, other Admin collections, production registration, or
production SQL/Data.
## Current bounded slice: authenticated AntiVirus.MaximumMessageSize mutation (2026-08-22)

Code/test commit `04403e1b7` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_MaximumMessageSize` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:359-367` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`avmaxmsgsize` setting, and returns `S_OK` on success. Net10 preserves the
existing Settings authentication boundary and COM identity, performs a
parameterized one-row update through `ISettingsAdministrationMutationStore`,
and updates the retained Settings snapshot only after persistence succeeds.

Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `345 passed, 0 skipped, 0
failed` focused and `2632 passed, 92 skipped, 0 failed` full Debug (`2724`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.
...
## Current bounded slice: authenticated AntiVirus.ClamAVEnabled mutation (2026-08-22)

Code/test commit `f1e9ecd81` closes one legacy Administrator mutation gap.
`InterfaceAntiVirus::put_ClamAVEnabled` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:464-475` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`ClamAVEnabled` setting as integer `0` or `1`, and returns `S_OK` on success.
Net10 preserves the existing Settings authentication boundary and COM
identity, performs a parameterized one-row update through
`ISettingsAdministrationMutationStore`, and updates the retained Settings
snapshot only after persistence succeeds.
...
Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `351 passed, 0 skipped, 0
failed` focused and `2638 passed, 92 skipped, 0 failed` full Debug (`2730`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.

Next independent slices, in order: establish or record the exact blocker for
Full-Text-capable disposable SQL Server `6000`; implement authenticated
`ClamAVHost` and `ClamAVPort` mutations separately; then continue isolated
...
Code/test commit `8f173c0ff` closes the final remaining setter in the current
Net10 AntiVirus COM surface. `InterfaceAntiVirus::put_ClamAVPort` in
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:519-533` returns access
denied before mutation for unauthenticated callers, writes the legacy fixed
`ClamAVPort` setting, and returns `S_OK` on success. Net10 preserves the
existing Settings authentication boundary and COM identity, performs a
...
Coverage is in `SettingsComContractTests.cs` and
`SqlServerSettingsAdministrationStoreTests.cs`: `357 passed, 0 skipped, 0
failed` focused and `2644 passed, 92 skipped, 0 failed` full Debug (`2736`
total). No live SQL acceptance was run because no approved disposable
Full-Text-capable SQL Server is available. Release remains RED.

Next independent slices, in order: establish or record the exact blocker for
Full-Text-capable disposable SQL Server `6000`; complete isolated registered
...
## Current authoritative status (2026-08-25, SMTP message backup admission)

Code/test commit `1b1493be5` wires the existing
`DeliveryQueuePauseDrainGate.EnterWorkerAsync` into normal
`SqlServerSmtpMessageReceiver.ReceiveAsync` calls. The lease spans SMTP policy
processing, event/rule/antivirus decisions, queue persistence, and the normal
delivery wake boundary. External-fetch requests deliberately skip nested
acquisition because `ExternalFetchProcessor.RunBatchAsync` already holds the
same batch lease through receiver execution and UID/account mutation.
...
`hmailserver/source/Server/SMTP/SMTPConnection.cpp:1120-1141`. Focused SMTP
receiver coverage passes `36/36`; standard full Debug passes `2735 passed, 96
skipped, 0 failed` (`2831` total).

Delivery queue, external-fetch, import, and normal SMTP writers are now
coordinated. IMAP, POP3, message COM, and lifecycle writers remain outside the
barrier. No full atomic SQL/Data, crash-consistency, or power-loss claim is
valid. Release remains **RED**. Next slice: one IMAP or POP3 persistence writer
with explicit negative/rollback coverage, or cloned installer/service/Data
rollback acceptance.
