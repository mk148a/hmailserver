...
Continue the Administrator object model with a bounded read-only `Settings -> Routes` slice: preserve the legacy `Routes`/`Route` COM dispatch and class identities, expose authenticated count/index/name/id lookup plus high-value non-secret core route scalars from existing `hm_routes` SQL data, and keep route-address collections, passwords, mutations, and deeper delivery behavior explicit `E_NOTIMPL`.
