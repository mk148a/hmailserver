...
Continue legacy Administrator parity with the next smallest safe slice: expose `Domains.Names` through the authenticated, read-only `Domains` collection using the already-loaded domain snapshots, matching the legacy tab-separated `id\tname\tactive\r\n` string shape. Preserve the installed vtable/DISPIDs; keep `Refresh`, collection mutation, database reload behavior, and broader domain mutation out of scope.
