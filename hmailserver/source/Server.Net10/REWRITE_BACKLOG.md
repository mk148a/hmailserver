...
Continue legacy Administrator parity with the next smallest safe slice: expose the existing `hm_domains.domainaddomain` value through the authenticated, read-only `Domain.ADDomainName` COM getter. Preserve the installed vtable/DISPIDs; keep the setter, AD synchronization, and authentication behavior out of scope.
