...
Continue legacy Administrator parity with the next bounded IMAP capability slice: expose read-only `Settings.IMAPSortEnabled`, `IMAPQuotaEnabled`, `IMAPIdleEnabled`, and `IMAPACLEnabled` from the existing `enableimapsort`, `enableimapquota`, `enableimapidle`, and `enableimapacl` `hm_settings.settinginteger` rows. Preserve installed vtable/DISPIDs and `VARIANT_BOOL` marshaling; keep setters, live IMAP capability/session changes, and broader Settings/Admin behavior out of scope.
