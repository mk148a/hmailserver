...
5. Security and anti-abuse modernization.
   ...
   - Done: DKIM evaluation-only foundation with the legacy result model (`Neutral`, `Pass`, `TempFail`, `PermFail`), deterministic `DKIM-Signature` tag parser, required-field validation, default/simple/relaxed canonicalization selection, signed-header list parsing, `b=` signature-value blanking, simple/relaxed body/header canonicalization tests, `bh=` body-hash verification over canonicalized body data including optional `l=` body-length handling and SHA1/SHA256 hash selection, injected-public-key header cryptographic verification for `rsa-sha1`/`rsa-sha256`, selector TXT public-key lookup behind an injected resolver, and message-level orchestration that extracts canonical header/body inputs, evaluates up to the legacy first five `DKIM-Signature` fields, skips unusable signatures as `Neutral`, and returns `Pass` when any signature verifies. Full DKIM verification only models `Pass` after both body hash and header signature verification succeed; mismatches, impossible body lengths, DNS temporary failures, missing/revoked/inapplicable key records, invalid public keys, and bad signatures map to the legacy result model. This does not perform SMTP rejection, policy scoring, signing, or Administrator setting wiring.
   - Remaining: DKIM policy/Admin wiring, DMARC; later Administrator/COM setting parity for SPF/greylisting knobs.
...
Continue DKIM with the next smallest safe policy slice: add a disabled-by-default DKIM policy boundary that consumes the message-level verifier result without changing SMTP accept/reject behavior unless explicitly enabled in tests. Keep signing, DMARC, Administrator/COM setting plumbing, and production policy defaults out of this slice.
