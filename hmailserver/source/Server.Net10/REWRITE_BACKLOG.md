...
Continue legacy Administrator parity with the next bounded forwarding-envelope Settings slice: expose read-only `Settings.RewriteEnvelopeFromWhenForwarding` from the configured/default legacy `hMailServer.ini` `[Settings] RewriteEnvelopeFromWhenForwarding` value, preserving the legacy disabled default, installed DISPID 107, and `VARIANT_BOOL` marshaling. Keep the setter, INI persistence, SMTP/rule forwarding behavior, DKIM signing interaction, and broader Settings/Admin mutation out of scope.
