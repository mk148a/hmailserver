...
Continue legacy Administrator parity with the next smallest safe slice: expose the existing `hm_domains.domainantispamoptions` greylisting flag through the authenticated, read-only `Domain.AntiSpamEnableGreylisting` COM getter. Preserve the installed vtable/DISPIDs; keep the setter, SMTP policy behavior, and runtime greylisting changes out of scope.
