...
Continue legacy Administrator parity with the next bounded Settings auto-ban scalar slice: expose read-only `Settings.AutoBanOnLogonFailure`, `MaxInvalidLogonAttempts`, `MaxInvalidLogonAttemptsWithin`, and `AutoBanMinutes` from existing `AutoBanOnLogonFailureEnabled`, `MaxInvalidLogonAttempts`, `LogonAttemptsWithinMinutes`, and `AutoBanMinutes` `hm_settings.settinginteger` rows. Preserve installed vtable/DISPIDs 82/83/84/85 and `VARIANT_BOOL`/integer marshaling; keep setters, `ClearLogonFailureList`, live auto-ban policy changes, security-range mutation behavior, and broader Settings/Admin mutation out of scope.
