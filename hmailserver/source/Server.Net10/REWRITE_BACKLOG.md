...
Resume the Administrator object model with a bounded read-only `Settings -> Directories` slice: preserve the legacy `Directories` COM identity/vtable contract, expose the authenticated Settings path using configured/default `hMailServer.ini` directory values (`ProgramDirectory`, `DatabaseDirectory`, `DataDirectory`, `LogDirectory`, `TempDirectory`, `EventDirectory`, and `DBScriptDirectory`) as read-only scalars, and keep direct activation, directory mutation, and persistence behavior explicit `E_ACCESSDENIED`/`E_NOTIMPL`.
