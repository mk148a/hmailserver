...
5. Security and anti-abuse modernization.
   ...
   - Remaining: DKIM Administrator/COM setting wiring and DMARC enforcement/Admin policy wiring; later Administrator/COM setting parity for SPF/greylisting knobs.
...
Continue legacy Administrator parity with the next smallest safe slice: expose the existing `hm_domains` DKIM configuration through authenticated, read-only `Domain` COM getters. Preserve the installed vtable/DISPIDs and enum values; keep all setters, signing behavior, private-key file-content access, and SMTP policy changes out of scope.
