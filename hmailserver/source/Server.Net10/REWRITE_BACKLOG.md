...
5. Security and anti-abuse modernization.
   ...
   - Done: DMARC evaluation-only foundation with an injected TXT resolver, deterministic record parser for `p`/`sp`/`aspf`/`adkim`/`pct`, RFC-shaped result model (`None`, `Pass`, `Fail`, `TempError`, `PermError`), exact-domain lookup with optional organizational-domain fallback, strict/relaxed SPF and DKIM alignment checks over caller-provided authentication results, subdomain policy selection, temporary DNS failure mapping, and malformed/duplicate record handling. This consumes SPF/DKIM outcomes only as data and does not perform SMTP reject/quarantine, spam scoring, signing, or Administrator/COM setting wiring.
   - Remaining: DKIM Administrator/COM setting wiring, DMARC SMTP/Admin policy wiring; later Administrator/COM setting parity for SPF/greylisting knobs.
...
Continue mail-authentication with the next smallest safe DMARC integration slice: add disabled-by-default SMTP DMARC policy/input plumbing over the existing evaluator, extending SPF/DKIM pass-domain surfaces only as needed, while preserving fail-open/no reject/quarantine defaults and keeping Administrator/COM setting plumbing out of the slice.
