...
Continue legacy Administrator parity with the next smallest safe slice: expose bounded read-only `Settings` protocol-enabled flags (`ServiceSMTP`, `ServicePOP3`, and `ServiceIMAP`) from the existing `protocolsmtp`, `protocolpop3`, and `protocolimap` `hm_settings.settinginteger` rows. Preserve the installed vtable/DISPIDs and `VARIANT_BOOL` marshaling; keep setters, live listener enable/disable, service state changes, and broader Settings/Admin behavior out of scope.
