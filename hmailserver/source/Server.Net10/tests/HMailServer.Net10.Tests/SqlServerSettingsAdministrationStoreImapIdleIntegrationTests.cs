...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreImapIdleIntegrationTests
{
    ...
    public async Task UpdateImapIdleEnabledAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_imapidle_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateImapIdleEnabledAsync(false, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(0, await ReadImapIdleEnabledAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task<int> ReadImapIdleEnabledAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'enableimapidle';",
            connection);
