...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreImapPublicFolderNameIntegrationTests
{
    ...
    public async Task UpdateImapPublicFolderNameAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_imappublic_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateImapPublicFolderNameAsync("#Shared", CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual("#Shared", await ReadImapPublicFolderNameAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task CreateSettingsTableAndSeedAsync(string connectionString)
    {
        const string sql = """
            ...
            VALUES (N'imappublicfoldername', N'#Public', 0);
            """;
    ...
    private static async Task<string> ReadImapPublicFolderNameAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settingstring FROM dbo.hm_settings WHERE settingname = N'imappublicfoldername';",
            connection);
