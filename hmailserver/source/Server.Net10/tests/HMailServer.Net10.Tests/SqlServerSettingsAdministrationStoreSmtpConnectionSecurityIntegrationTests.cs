...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreSmtpConnectionSecurityIntegrationTests
{
    ...
    public async Task UpdateSmtpConnectionSecurityAsync_PersistsFixedRowValuesAndReturnsFalseWhenMissing()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_smtpsecurity_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            foreach (var expectedValue in new[] { 0, 1, 2, 3 })
            {
                Assert.IsTrue(
                    await store.UpdateSmtpConnectionSecurityAsync(expectedValue, CancellationToken.None)
                        .ConfigureAwait(false));
                Assert.AreEqual(
                    expectedValue,
                    await ReadSmtpConnectionSecurityAsync(testConnectionString).ConfigureAwait(false));
            }

            await DeleteSmtpConnectionSecurityRowAsync(testConnectionString).ConfigureAwait(false);

            Assert.IsFalse(
                await store.UpdateSmtpConnectionSecurityAsync(0, CancellationToken.None).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task CreateSettingsTableAndSeedAsync(string connectionString)
    {
        const string sql = """
            ...
            VALUES (N'SmtpDeliveryConnectionSecurity', N'', 2);
            """;
    ...
    private static async Task<int> ReadSmtpConnectionSecurityAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'SmtpDeliveryConnectionSecurity';",
            connection);
        return Convert.ToInt32(await command.ExecuteScalarAsync().ConfigureAwait(false));
    }

    private static async Task DeleteSmtpConnectionSecurityRowAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "DELETE FROM dbo.hm_settings WHERE settingname = N'SmtpDeliveryConnectionSecurity';",
            connection);
        await command.ExecuteNonQueryAsync().ConfigureAwait(false);
    }
