...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreAntiSpamMxChecksIntegrationTests
{
    ...
    public async Task UpdateAntiSpamUseMxChecksAsync_PersistsBothLegacyRowsAndFailsClosedWhenMissing()
    {
        ...
        var databaseName = $"hmailserver_net10_antispammx_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(await store.UpdateAntiSpamUseMxChecksAsync(true, CancellationToken.None));
            Assert.IsTrue(await store.UpdateAntiSpamUseMxChecksScoreAsync(8, CancellationToken.None));
            CollectionAssert.AreEqual(new[] { 1, 8 }, await ReadValuesAsync(testConnectionString));

            await DeleteRowAsync(testConnectionString, "usemxchecks");
            Assert.IsFalse(await store.UpdateAntiSpamUseMxChecksAsync(false, CancellationToken.None));
        }
        finally
        {
    ...
    private static async Task CreateSettingsTableAndSeedAsync(string connectionString)
    {
        const string sql = """
            ...
            VALUES (N'usemxchecks', N'', 0), (N'usemxchecksscore', N'', 2);
            """;
    ...
    private static async Task<int[]> ReadValuesAsync(string connectionString)
    {
        const string sql = """
            ...
            WHERE settingname IN (N'usemxchecks', N'usemxchecksscore')
