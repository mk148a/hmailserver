...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreImapAclIntegrationTests
{
    ...
    public async Task UpdateImapAclEnabledAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_imapacl_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateImapAclEnabledAsync(true, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(1, await ReadImapAclEnabledAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task<int> ReadImapAclEnabledAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'enableimapacl';",
            connection);
