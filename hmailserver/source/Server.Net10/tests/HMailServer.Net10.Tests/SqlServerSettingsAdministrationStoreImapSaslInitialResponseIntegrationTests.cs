...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreImapSaslInitialResponseIntegrationTests
{
    ...
    public async Task UpdateImapSaslInitialResponseEnabledAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_imapsaslinitial_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateImapSaslInitialResponseEnabledAsync(true, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(1, await ReadImapSaslInitialResponseEnabledAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task CreateSettingsTableAndSeedAsync(string connectionString)
    {
        const string sql = """
            ...
            VALUES (N'EnableImapSASLInitialResponse', N'', 0);
            """;
    ...
    private static async Task<int> ReadImapSaslInitialResponseEnabledAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'EnableImapSASLInitialResponse';",
            connection);
