...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreWelcomeSmtpIntegrationTests
{
    ...
    public async Task UpdateWelcomeSmtpAsync_PersistsAWelcomeSmtpValueLongerThan255Characters()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_welcomesmtp_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            var welcomeSmtp = "220 " + new string('x', 296);
            Assert.IsTrue(welcomeSmtp.Length > 255 && welcomeSmtp.Length <= 4000);

            var store = new SqlServerSettingsAdministrationStore(
                new SqlServerConnectionFactory(testConnectionString));
            Assert.IsTrue(
                await store.UpdateWelcomeSmtpAsync(welcomeSmtp, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(welcomeSmtp, await ReadWelcomeSmtpAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task CreateSettingsTableAndSeedAsync(string connectionString)
    {
        const string sql = """
            ...
            VALUES (N'welcomesmtp', N'220 initial', 0);
            """;
    ...
    private static async Task<string> ReadWelcomeSmtpAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settingstring FROM dbo.hm_settings WHERE settingname = N'welcomesmtp';",
            connection);
