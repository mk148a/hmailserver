...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreImapSaslPlainIntegrationTests
{
    ...
    public async Task UpdateImapSaslPlainEnabledAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_imapsaslplain_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateImapSaslPlainEnabledAsync(false, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(0, await ReadImapSaslPlainEnabledAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task CreateSettingsTableAndSeedAsync(string connectionString)
    {
        const string sql = """
            ...
            VALUES (N'EnableImapSASLPlain', N'', 1);
            """;
    ...
    private static async Task<int> ReadImapSaslPlainEnabledAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'EnableImapSASLPlain';",
            connection);
