...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreImapMasterUserIntegrationTests
{
    ...
    public async Task UpdateImapMasterUserAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_imapmaster_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateImapMasterUserAsync("new-master", CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual("new-master", await ReadImapMasterUserAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task<string> ReadImapMasterUserAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settingstring FROM dbo.hm_settings WHERE settingname = N'ImapMasterUser';",
            connection);
