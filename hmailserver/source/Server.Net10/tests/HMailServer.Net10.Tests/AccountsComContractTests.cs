    ...
    public void AuthorizedCollection_ExposesOnlyReadOnlySnapshotAndLegacyLookupErrors()
    {
        IInterfaceAccounts accounts = Accounts.CreateAuthorized(
            new[]
            {
                ...
                new AccountAdministrationSnapshot(20, 100, "user@example.test", false, 0)
            });

        Assert.AreEqual(2, accounts.Count);
        AssertAccount(accounts[0], 10, 100, "admin@example.test", true, ComAdminLevel.ServerAdministrator);
        AssertAccount(accounts.get_ItemByAddress("USER@EXAMPLE.TEST"), 20, 100, "user@example.test", false, ComAdminLevel.Normal);
        AssertAccount(accounts.get_ItemByDBID(10), 10, 100, "admin@example.test", true, ComAdminLevel.ServerAdministrator);
