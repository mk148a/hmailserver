...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreServicePop3IntegrationTests
{
    ...
    public async Task UpdateServicePop3Async_PersistsProtocolPop3Setting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_servicepop3_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateServicePop3Async(false, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(0, await ReadServicePop3Async(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task<int> ReadServicePop3Async(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'protocolpop3';",
            connection);
