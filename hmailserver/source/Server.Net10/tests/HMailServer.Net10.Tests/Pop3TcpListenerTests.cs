    ...
    public async Task LoopbackAcceptLatency_ServesBannerWithinBudget()
    {
        ...
        try
        {
            for (var i = 0; i < clientCount; i++)
            {
                ...
                stopwatch.Stop();
                Assert.AreEqual("+OK hMailServer .NET 10 POP3 ready", banner);
            ...
            Assert.IsTrue(p95 < TimeSpan.FromSeconds(5).TotalMilliseconds, "POP3" + " accept p95 (" + p95.ToString("0.###") + "ms) exceeded budget.");
        }
        finally
        {
            await StopListenerAsync(runTask, cts);
