...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreServiceSmtpIntegrationTests
{
    ...
    public async Task UpdateServiceSmtpAsync_PersistsProtocolSmtpSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_servicesmtp_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateServiceSmtpAsync(false, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(0, await ReadServiceSmtpAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task<int> ReadServiceSmtpAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'protocolsmtp';",
            connection);
