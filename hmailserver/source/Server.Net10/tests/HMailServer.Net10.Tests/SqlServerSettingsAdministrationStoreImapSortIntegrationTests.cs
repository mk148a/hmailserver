...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreImapSortIntegrationTests
{
    ...
    public async Task UpdateImapSortEnabledAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_imapsort_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateImapSortEnabledAsync(false, CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual(0, await ReadImapSortEnabledAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task<int> ReadImapSortEnabledAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settinginteger FROM dbo.hm_settings WHERE settingname = N'enableimapsort';",
            connection);
