...
""",
    ...
    public async Task AuthenticatedComPath_ReadsBoundedSettingsScalarsFromIsolatedDatabase()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(0, cache.DomainHitRate);
            Assert.AreEqual(0, cache.AccountHitRate);
            Assert.AreEqual(0, cache.AliasHitRate);
            Assert.AreEqual(0, cache.DistributionListHitRate);
            Assert.AreEqual(0, cache.DomainCacheMaxSizeKb);
            Assert.AreEqual(0, cache.DomainCacheSizeKb);
            Assert.AreEqual(0, cache.AccountCacheMaxSizeKb);
            Assert.AreEqual(0, cache.AccountCacheSizeKb);
            Assert.AreEqual(0, cache.AliasCacheMaxSizeKb);
            Assert.AreEqual(0, cache.AliasCacheSizeKb);
            Assert.AreEqual(0, cache.DistributionListCacheMaxSizeKb);
            Assert.AreEqual(0, cache.DistributionListCacheSizeKb);
            cache.Clear();
