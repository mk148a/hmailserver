...
[TestClass]
[DoNotParallelize]
public sealed class SqlServerSettingsAdministrationStoreHostNameIntegrationTests
{
    ...
    public async Task UpdateHostNameAsync_PersistsExistingSetting()
    {
        var serverConnectionString = GetApprovedConnectionStringOrInconclusive();
        var databaseName = $"hmailserver_net10_hostname_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            Assert.IsTrue(
                await store.UpdateHostNameAsync("mail.example.test", CancellationToken.None).ConfigureAwait(false));

            Assert.AreEqual("mail.example.test", await ReadHostNameAsync(testConnectionString).ConfigureAwait(false));
        }
        finally
        {
    ...
    private static async Task CreateSettingsTableAndSeedAsync(string connectionString)
    {
        const string sql = """
            ...
            VALUES (N'hostname', N'old.example.test', 0);
            """;
    ...
    private static async Task<string> ReadHostNameAsync(string connectionString)
    {
        await using var connection = new SqlConnection(connectionString);
        await connection.OpenAsync().ConfigureAwait(false);
        await using var command = new SqlCommand(
            "SELECT settingstring FROM dbo.hm_settings WHERE settingname = N'hostname';",
            connection);
