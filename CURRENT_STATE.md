...
- Current bounded slice: authenticated `Settings.SMTPRelayerRequiresAuthentication` (`DispId(34)`, `VARIANT_BOOL`) persists only the fixed existing `hm_settings.usesmtprelayerauthentication` row through a parameterized integer update, rechecks the live administrator callback, and publishes the retained snapshot only after one-row success; fixed-relayer credential selection, SMTP routing, change notifications, and reconfiguration remain unchanged
...
- Next three independent slices: (1) fresh legacy-first audit of the next low-risk Settings mutation after SMTPRelayerRequiresAuthentication, (2) execute settings/message rollback against an approved disposable SQL/Data target, (3) non-DB combined settings restore containment/order decision
