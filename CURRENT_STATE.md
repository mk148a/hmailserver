...
- Current HEAD: `b4cacd531`
- Last successfully pushed commit: `0b2646731`
- Latest focused-test result: settings COM/store tests 39 passed, 0 failed, 0 skipped
- Latest full Net10 result: `2022 passed, 39 skipped, 0 failed`
...
- Current bounded slice: authenticated `Settings.MaxSMTPRecipientsInBatch` (`DispId(62)`) now persists only the fixed existing `hm_settings.maxsmtprecipientsinbatch` row through a parameterized update, rechecks the live administrator callback, and publishes the retained snapshot only after one-row success; DefaultDomain, MirrorEMailAddress, WorkerThreadPriority, MaxSMTPConnections, MaxPOP3Connections, WelcomePOP3, WelcomeIMAP, and WelcomeSMTP use the same bounded seam; delivery batching runtime reconfiguration and other setters remain unchanged
...
- Next three independent slices: (1) fresh legacy-first audit of the next low-risk Settings mutation after MaxSMTPRecipientsInBatch, (2) execute settings/message rollback against an approved disposable SQL/Data target, (3) non-DB combined settings restore containment/order decision
