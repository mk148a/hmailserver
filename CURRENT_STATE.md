...
- Current bounded slice: authenticated `Settings.SMTPMinutesBetweenTry` (`DispId(20)`) now persists only the fixed existing `hm_settings.smtpminutesbetweenretries` row through a parameterized integer update, rechecks the live administrator callback, and publishes the retained snapshot only after one-row success; the previously completed fixed-row settings mutations use the same bounded seam; `ExternalDelivery` retry scheduling and runtime reconfiguration remain unchanged
...
- Next three independent slices: (1) fresh legacy-first audit of the next low-risk Settings mutation after SMTPMinutesBetweenTry, (2) execute settings/message rollback against an approved disposable SQL/Data target, (3) non-DB combined settings restore containment/order decision
