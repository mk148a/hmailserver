...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Account -> FetchAccounts` dilimiyle devam etmek. Legacy `FetchAccounts`/`FetchAccount` COM dispatch kontrati korunmali; authenticated `Account -> FetchAccounts` count/index/id lookup mevcut `hm_fetchaccounts` SQL verisinden gelmeli. Direct activation, mutation, password/security-sensitive alanlar ve fetch execution behavior acik `E_NOTIMPL` kalmali.
