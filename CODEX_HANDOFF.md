...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Domain -> Accounts` dilimiyle devam etmek. Once legacy `Accounts` collection identity/dispatch kontrati korunmali; ardindan yalniz authenticated account count/item/address lookup secili domain icin mevcut SQL verisi uzerinden acilmali. Account mutation'lari ve derin account child collection uyeleri kendi testli dilimlerine kadar acik `E_NOTIMPL` kalmali.
