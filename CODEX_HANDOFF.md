...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Account -> Rules` dilimiyle devam etmek. Legacy `Rules`/`Rule` COM dispatch kontrati korunmali; authenticated account-rule count/index/id lookup ve yuksek degerli rule core scalar'lari mevcut SQL verisinden gelmeli. Criteria/actions, script execution, mutation ve derin davranislar acik `E_NOTIMPL` kalmali.
