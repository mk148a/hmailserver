...
## Current Authoritative Continuation (2026-08-22, atomic administrator-password INI persistence)

Code/test commit `d00214ec3` hardens `Settings.SetAdministratorPassword`.
Legacy `hmailserver/source/Server/Common/Application/IniFileSettings.cpp:352-366`
hashes the new password and writes only the Security INI value through
`WritePrivateProfileString`. Net10 now writes a same-directory `.ini`
snapshot, flushes the profile API and file, atomically replaces the target,
and removes the temporary sibling. Focused initialization/settings tests are
`18 passed, 0 skipped, 0 failed`; full Debug Net10 is `2687 passed, 94 skipped,
0 failed` (`2781` total). No COM identity, SQL/Data, service, registration,
DCOM, IIS, firewall, or pre-existing artifact changed. Power-loss injection,
reboot durability, native crash semantics, Full-Text SQL/Data, installed COM,
migration/restore, SEC-18, installer/service/Data rollback, paired C++
performance, protocol thresholds, and 24-hour soak remain unproven. Release
remains RED. Next: approved Full-Text SQL/Data round-trip acceptance. Older
...
## Historical Authoritative Continuation (2026-08-22, JScript line-separator parity)
...
not permit disposable symlink creation, and fail `0`; full Debug Net10 passes
`2652`, skips `94`, and fails `0` (`2746` total). Release remains **RED**:
...
and 24-hour soak, and private binding handle-based TOCTOU protection remain
open or environment-blocked. Next: approved Full-Text
...
## Historical Authoritative Continuation (2026-08-22, handle-relative raw DataBackup snapshot)
...
The canonical digest framing, ordering, UTF-8 encoding, and uppercase SHA-256
output remain compatible. Legacy anchors are
`hmailserver/source/Server/Common/Application/BackupExecuter.cpp:195-217` and
`hmailserver/source/Server/Common/Util/FileUtilities.cpp:370-402`; focused
coverage includes nested, empty, zero-byte, Unicode, and tamper cases.

Focused `BackupArchiveIdentityTests` pass `7`, skip `2` because disposable
symlink creation is unavailable, and fail `0`; full Debug Net10 passes `2653`,
skips `94`, and fails `0` (`2747` total). No COM identity, SQL/Data schema,
service, registry, DCOM, IIS, firewall, or production state changed. Release
remains **RED**: the handle-relative digest is not an atomic filesystem
snapshot; same-name replacement and destination ancestor TOCTOU review remain
open, as do Full-Text SQL/Data, registered COM/SEC-18, installer rollback,
remaining COM/Admin parity, paired C++/.NET performance, protocol thresholds,
and 24-hour soak.
...
`InterfaceAntiVirus::put_ClamAVPort` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:519-533`. The Net10
...
uses the fixed `hm_settings` row `ClamAVPort` in
...
`2644`, skips `92`, and fails `0` (`2736` total). No production service,
...
Code/test commit `d9db97814` closes the legacy authenticated
`InterfaceAntiVirus::put_ClamAVHost` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:484-508`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `ClamAVHost` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `354`, skip `0`, and fail `0`; default full Net10 passes
`2641`, skips `92`, and fails `0` (`2733` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
disposable SQL Server `6000` gate once, then complete `ClamAVPort` parity.

## Current Authoritative Continuation (2026-08-22, ClamAV enabled mutation)

Code/test commit `f1e9ecd81` closes the legacy authenticated
`InterfaceAntiVirus::put_ClamAVEnabled` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:451-475`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `ClamAVEnabled` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `351`, skip `0`, and fail `0`; default full Net10 passes
`2638`, skips `92`, and fails `0` (`2730` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
disposable SQL Server `6000` gate once, then implement `ClamAVHost` and
`ClamAVPort` as separate bounded slices.

## Current Authoritative Continuation (2026-08-22, attachment-blocking mutation)

Code/test commit `b2c2314e5` closes the legacy authenticated
`InterfaceAntiVirus::put_EnableAttachmentBlocking` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:400-424`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `enableattachmentblocking` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `348`, skip `0`, and fail `0`; default full Net10 passes
`2635`, skips `92`, and fails `0` (`2727` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
disposable SQL Server `6000` gate once, then continue the remaining ClamAV
Admin mutations.

## Current Authoritative Continuation (2026-08-22, AntiVirus MaximumMessageSize mutation)

Code/test commit `04403e1b7` closes the legacy authenticated
`InterfaceAntiVirus::put_MaximumMessageSize` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:343-367`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `avmaxmsgsize` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `345`, skip `0`, and fail `0`; default full Net10 passes
`2632`, skips `92`, and fails `0` (`2724` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
...
## Current Authoritative Continuation (2026-08-22, CustomScannerReturnValue mutation)

Code/test commit `3f66a8eb9` closes the legacy authenticated
`InterfaceAntiVirus::put_CustomScannerReturnValue` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:206-221`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `customviursscannerreturnvalue` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `342`, skip `0`, and fail `0`; default full Net10 passes
`2629`, skips `92`, and fails `0` (`2721` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
...
## Current Authoritative Continuation (2026-08-22, CustomScannerExecutable mutation)

Code/test commit `f02091d5f` closes the legacy authenticated
`InterfaceAntiVirus::put_CustomScannerExecutable` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:172-189`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `customvirusscannerexecutable` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `339`, skip `0`, and fail `0`; default full Net10 passes
`2626`, skips `92`, and fails `0` (`2718` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
...
## Current Authoritative Continuation (2026-08-22, CustomScannerEnabled mutation)

Code/test commit `40e77eb2b` closes the legacy authenticated
`InterfaceAntiVirus::put_CustomScannerEnabled` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:138-157`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `usecustomvirusscanner` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `336`, skip `0`, and fail `0`; default full Net10 passes
`2623`, skips `92`, and fails `0` (`2715` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
...
## Current Authoritative Continuation (2026-08-22, AntiVirus NotifySender mutation)

Code/test commit `1444bea7e` closes the legacy authenticated
`InterfaceAntiVirus::put_NotifySender` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:292-309`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `avnotifysender` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `333`, skip `0`, and fail `0`; default full Net10 passes
`2620`, skips `92`, and fails `0` (`2712` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
...
`InterfaceAntiVirus::put_Action` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:223-276`. The Net10
setter is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `avaction` in
`SqlServerSettingsAdministrationStore.cs`. The two installed enum values map
to the legacy integers `0` and `1`; the mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `327`, skip `0`, and fail `0`; default full Net10 passes
`2614`, skips `92`, and fails `0` (`2706` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
...
`InterfaceAntiVirus::put_ClamWinExecutable` gap from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:70-85`. The Net10
implementation is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `avclamwinexec` in
`SqlServerSettingsAdministrationStore.cs`. The setter is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `321`, skip `0`, and fail `0`; default full Net10 passes
`2608`, skips `92`, and fails `0` (`2700` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
disposable SQL Server `6000` gate once, then implement the next smallest
authenticated `ClamWinDBFolder` mutation.

## Current Authoritative Continuation (2026-08-22, authenticated AntiVirus mutation)

Code/test commit `e43f27997` implements the bounded legacy
`InterfaceAntiVirus::put_ClamWinEnabled` behavior from
`hmailserver/source/Server/COM/InterfaceAntiVirus.cpp:23-44`. The Net10 setter
is in `hmailserver/source/Server.Net10/src/HMailServer.ComInterop/AntiVirus.cs`;
the Settings-owned retained snapshot is published by
`hmailserver/source/Server.Net10/src/HMailServer.ComInterop/Settings.cs`; SQL
uses the fixed `hm_settings` row `avclamwinenable` in
`SqlServerSettingsAdministrationStore.cs`. The mutation is authenticated,
lease-bound, parameterized, and publishes only after exactly one row updates.
Installed COM identity and direct activation boundaries are unchanged.

Focused tests pass `318`, skip `0`, and fail `0`; default full Net10 passes
`2605`, skips `92`, and fails `0` (`2697` total). No production service,
database, Data directory, registration, DCOM ACL, IIS, or firewall state was
changed. Release remains RED because disposable Full-Text SQL/Data round-trip,
installer rollback, registered COM/SEC-18, protocol/load parity, and soak
evidence remain open or environment-blocked. Next: verify the Full-Text-capable
disposable SQL Server `6000` gate once, then take the next smallest
authenticated AntiVirus/Admin mutation slice.
...
fixed-row update, fails closed when the lease is unavailable, disposes it on
success/failure, and publishes the retained snapshot only after success.
Focused tests pass `4`, skip `0`, and fail `0`; full Net10 passes `2591`, skips
`90`, and fails `0` (`2681` total). No COM identity, SQL schema,
default-domain runtime, live reconfiguration, service, registry, DCOM, IIS,
or production state changed.

Next slice: audit the next remaining authenticated Settings/Admin mutation
lease gap, prioritizing `AddDeliveredToHeader` or `MaxNumberOfMXHosts`.
Release remains RED; disposable SQL/Data, SEC-18, registered COM, paired C++
performance, and long-soak evidence remain open or environment-blocked.
...
fixed-row update, fails closed when the lease is unavailable, disposes it on
success/failure, and publishes the retained snapshot only after success.
Focused tests pass `4`, skip `0`, and fail `0`; full Net10 passes `2585`, skips
`90`, and fails `0` (`2675` total). No COM identity, SQL schema, asynchronous
task runtime, live reconfiguration, service, registry, DCOM, IIS, or
production state changed.

Next slice: audit one remaining authenticated Settings mutation lease gap,
starting with `MaxMessageSize`. Release remains RED; disposable SQL/Data,
SEC-18, registered COM, paired C++ performance, and long-soak evidence remain
open or environment-blocked.
...
`hmailserver/source/Server/COM/InterfaceBackupSettings.cpp:189-220` and
delegates to `Configuration::SetBackupOption` at
`hmailserver/source/Server/Common/Application/Configuration.cpp:450-472` for
`HM::Backup::BOCompression = 8` from
`hmailserver/source/Server/Common/Application/Backup.h:14-20`.

Net10 preserves IID `2C5559F0-DF3F-43C0-935C-F79D41CF8A5B`, DISPID `5`,
`VARIANT_BOOL`, CLSID, ProgID, and vtable order. The SQL store atomically sets
or clears only bit `8` in the current `hm_settings.backupoptions` row. The
...
publication merges bit `8` against current parent options for retained-child
safety.

Focused tests pass `282/282`; full Net10 passes `2569`, skips `90`, and fails
`0` (`2659` total). Next code slice: raw non-DB-only `BODomains|BOMessages`
DataBackup staging, leaving the external `DataBackup` directory beside the
archive. Release remains RED because backup execution, disposable SQL/Data
...
`hmailserver/source/Server/COM/InterfaceBackupSettings.cpp:120-152` and
delegates to `Configuration::SetBackupOption` at
`hmailserver/source/Server/Common/Application/Configuration.cpp:450-474` for
`HM::Backup::BODomains = 2` from
`hmailserver/source/Server/Common/Application/Backup.h:14-20`.

Net10 preserves IID `2C5559F0-DF3F-43C0-935C-F79D41CF8A5B`, DISPID `3`,
`VARIANT_BOOL`, CLSID, ProgID, and vtable order. The SQL store atomically sets
or clears only bit `2` in the current `hm_settings.backupoptions` row. The
existing authenticated owning Settings boundary and authorization lease
cover the write; failed or zero-row writes do not publish snapshots.

Focused tests pass `272/272`; full Net10 passes `2559`, skips `90`, and fails
`0` (`2649` total). Next code slice: `BackupMessages` bit `4` parity. Release
remains RED because backup execution, disposable SQL/Data host-start and
restore, rollback, SEC-18, registered/out-of-process COM, paired C++
performance, and long-soak evidence remain open.
...
which reads and rewrites only flag `16` in `hm_settings.SslVersions`. Code/test
commit `0f974cdd9` adds the authenticated Net10 bit-16 mutation, unrelated-bit
preservation, fail-closed outcome, and publish-after-success snapshot behavior.
Focused Settings/SQL tests pass `248`, fail `0`; full Net10 passes `2527`,
skips `90`, fails `0` (`2617` total).
...
## Current Authoritative Continuation (2026-08-21, TLS 1.2 flag persistence)

Legacy `InterfaceSettings::put_TlsVersion12Enabled`
(`hmailserver/source/Server/COM/InterfaceSettings.cpp:2359-2372`) calls
`Configuration::SetSslVersionEnabled`
(`hmailserver/source/Server/Common/Application/Configuration.cpp:632-640`),
which reads and rewrites only flag `8` in `hm_settings.SslVersions`. Code/test
commit `01fa372ff` adds the authenticated Net10 bit-8 mutation, unrelated-bit
preservation, fail-closed outcome, and publish-after-success snapshot behavior.
Focused Settings/SQL tests pass `247`, fail `0`; full Net10 passes `2526`,
skips `90`, fails `0` (`2616` total).
...
`Configuration::SetSslVersionEnabled`
(`hmailserver/source/Server/Common/Application/Configuration.cpp:632-640`),
which reads and rewrites only flag `2` in `hm_settings.SslVersions`, seeded by
`CreateTablesMSSQL.sql:940`. Code/test commit `039bfb9fc` adds the authenticated
Net10 parameterized read-modify-write, unrelated-bit preservation, fail-closed
outcome, and publish-after-success snapshot behavior. Focused Settings/SQL
tests pass `245`, fail `0`; full Net10 passes `2524`, skips `90`, fails `0`
(`2614` total).
...
updates only the existing `imappublicfoldername` row, requires the existing
authenticated server-administrator boundary and authorization lease, publishes
the snapshot after a successful one-row update, and keeps direct activation
fallback behavior. Focused COM/SQL coverage is `224 passed, 0 skipped, 0
failed`; disposable LocalDB SQL integration passed; full Net10 Debug is `2542
passed, 10 skipped, 0 failed` (`2552` total).

Legacy references are `InterfaceSettings::get/put_IMAPPublicFolderName`
(`source/Server/COM/InterfaceSettings.cpp:1304-1332`) and
`IMAPConfiguration::Get/SetIMAPPublicFolderName`
(`source/Server/IMAP/IMAPConfiguration.cpp:126-134`). No installed COM
identity, direct activation boundary, SMTP trust behavior, IMAP ACL behavior,
or authentication was changed. The next slice is IMAPHierarchyDelimiter
...
`Settings.IMAPSASLPlainEnabled` persistence through
`SqlServerSettingsAdministrationStore.UpdateImapSaslPlainEnabledAsync`. It
updates only the existing `EnableImapSASLPlain` row, requires the existing
authenticated server-administrator boundary and authorization lease, publishes
the snapshot after a successful one-row update, and keeps direct activation
fallback behavior. Focused COM/SQL coverage is `220 passed, 0 skipped, 0
failed`; disposable LocalDB SQL integration passed; full Net10 Debug is `2536
passed, 10 skipped, 0 failed` (`2546` total).

Legacy references are `InterfaceSettings::get/put_IMAPSASLPlainEnabled`
(`source/Server/COM/InterfaceSettings.cpp:2529-2553`) and
`IMAPConfiguration::Get/SetUseIMAPSASLPlain`
(`source/Server/IMAP/IMAPConfiguration.cpp:150-158`). No installed COM
identity, direct activation boundary, SMTP trust behavior, credentials, or
live IMAP capability was changed. The next slice is
IMAPSASLInitialResponseEnabled persistence parity. Migration/installer,
SEC-18, paired C++ performance, and 24-hour soak remain open; release is
`RED`; no push was performed.
