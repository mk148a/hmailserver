...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Domain` core-scalar dilimiyle devam etmek. Mevcut authenticated SQL-backed `Domain` snapshot'i yalniz `hm_domains` tablosunda zaten bulunan yuksek degerli detail-screen scalar'lari icin genisletilmeli (ornegin postmaster ile quota/limit enable/value alanlari). Legacy `Domain` dispatch kontrati korunmali; scalar mutation'lari ve core olmayan alanlar acik `E_NOTIMPL` kalmali.
