...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Settings -> Routes` dilimiyle devam etmek. Legacy `Routes`/`Route` COM dispatch ve class identity kontratlari korunmali; authenticated count/index/name/id lookup ve yuksek degerli non-secret core route scalar'lari mevcut `hm_routes` SQL verisinden gelmeli. Route-address alt koleksiyonu, parola, mutation ve derin teslimat davranisi acik `E_NOTIMPL` kalmali.
