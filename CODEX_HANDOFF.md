...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Account` delivery/detail-scalar dilimiyle devam etmek. Mevcut authenticated SQL-backed `Account` adapter'i yalniz `hm_accounts` tablosunda zaten bulunan non-secret delivery/autoreply/signature scalar'lari icin genisletilmeli; password/security-sensitive alanlar, behavior execution ve child collection'lar bu dilimde disarida kalmali. Legacy `Account` dispatch kontrati korunmali; scalar mutation'lari ve core olmayan alanlar acik `E_NOTIMPL` kalmali.
