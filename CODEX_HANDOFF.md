...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Domain -> Aliases` dilimiyle devam etmek. Once legacy `Aliases` ve `Alias` collection/object identity/dispatch kontratlari korunmali; ardindan yalniz authenticated alias count/item/name/id lookup secili domain icin mevcut SQL verisi uzerinden acilmali. Alias mutation'lari ve derin object davranislari kendi testli dilimlerine kadar acik `E_NOTIMPL` kalmali.
