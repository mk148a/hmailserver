...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Domain -> DistributionLists` dilimiyle devam etmek. Once legacy `DistributionLists` ve `DistributionList` collection/object identity/dispatch kontratlari korunmali; ardindan yalniz authenticated list count/item/address/id lookup secili domain icin mevcut SQL verisi uzerinden acilmali. List mutation'lari, recipients ve derin object davranislari kendi testli dilimlerine kadar acik `E_NOTIMPL` kalmali.
