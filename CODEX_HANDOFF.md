...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Settings -> PublicFolders` kok dilimiyle devam etmek. Korunmus legacy `IMAPFolders`/`IMAPFolder` COM kontratlari yeniden kullanilmali; authenticated public-root folder count/index/name/id lookup ve ayni read-only core scalar'lar `hm_imapfolders` icinde `folderaccountid = 0` verisinden gelmeli. Messages, permissions, nested subfolders, mutation ve derin davranislar acik `E_NOTIMPL` kalmali.
