...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Application -> Database` dilimiyle devam etmek. Legacy `Database` COM kontrati korunmali; authenticated Application yolu yalniz mevcut SQL/runtime configuration'dan kanitlanabilen non-mutating database/version/configuration scalar'larini acmali. Direct activation ve SQL execution, transaction, database create/default-selection, script execution, message filename utility, prerequisite ve persistence operasyonlari acik `E_ACCESSDENIED`/`E_NOTIMPL` kalmali.
...
5. Current Next Slice olarak `Application -> Database` read-only COM/Admin dilimini ele al; once eksik legacy contract ve guvenli non-mutating database/config scalar davranisini kanitlayan dar test yaz.
