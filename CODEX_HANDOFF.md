...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Account -> IMAPFolders` dilimiyle devam etmek. Legacy `IMAPFolders`/`IMAPFolder` COM dispatch ve class identity kontratlari korunmali; authenticated top-level folder count/index/name/id lookup ve yuksek degerli core folder scalar'lari mevcut `hm_imapfolders` SQL verisinden gelmeli. Messages, permissions, nested subfolders, mutation ve derin davranislar acik `E_NOTIMPL` kalmali.
