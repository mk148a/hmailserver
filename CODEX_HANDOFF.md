...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Account` core-scalar dilimiyle devam etmek. Mevcut authenticated SQL-backed `Account` adapter'i yalniz `hm_accounts` tablosunda zaten bulunan yuksek degerli detail-screen scalar'lari icin genisletilmeli; password/security-sensitive alanlar ve behavior-heavy child collection'lar bu dilimde disarida kalmali. Legacy `Account` dispatch kontrati korunmali; scalar mutation'lari ve core olmayan alanlar acik `E_NOTIMPL` kalmali.
