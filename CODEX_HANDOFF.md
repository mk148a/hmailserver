...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Domain -> DomainAliases` dilimiyle devam etmek. Once legacy `DomainAliases` ve `DomainAlias` collection/object identity/dispatch kontratlari korunmali; ardindan yalniz authenticated count/item/id lookup ve read-only alias/domain-id scalar'lari secili domain icin mevcut `hm_domain_aliases` SQL verisi uzerinden acilmali. Domain-alias mutation'lari kendi testli dilimine kadar acik `E_NOTIMPL` kalmali.
