...
Backlog'daki siradaki ana dilim: legacy `Settings` interface'inin tam vtable sirasini koruyup authenticated `MessageIndexing` property sini uygulamak, administrator hash'ini configured/default legacy `hMailServer.ini` dosyasindan yuklemek ve ancak bu yol tamamlandiktan sonra real `Application` CLSID factory'sini service-process host'a kaydetmek. Diger Settings uyeleri store'lari gelene kadar acik `E_NOTIMPL`, direct child CLSID activation access-denied kalmali; registry/type-library/ProgID wiring'i service-install diliminde tamamlanmali.

Son tamamlanan kucuk dilimler:

- Legacy installed 5.7 type library ile karsilastirilarak dual `Application` ve `Account` IID'leri, tam 20/61-member vtable sirasi, CLSID/versioned ProgID/default-interface metadata'si, server-admin account sonucu ve detached-account access-denied siniri korundu. Registry'siz `CoCreateInstance` testi root IID uzerinden server-admin authentication'i dogruluyor. Real Application CLSID service'e henuz kayitli degil; authenticated Settings tam kontrat gelene kadar `E_NOTIMPL`.
- Server-admin credential provider case-insensitive `Administrator`, legacy MD5/salted-SHA256 hash, wrong credential ve empty-hash/empty-password anonymous administration davranislarini koruyor. Service'in configured/default `hMailServer.ini` dosyasindan hash yukleme wiring'i sonraki dilimde.
- Windows service, dedicated MTA thread uzerinde suspended `CoRegisterClassObject` kayitlarini topluca resume eden ve shutdown'da ayni thread'de revoke eden process-local COM class-factory host'u calistiriyor. Registry'ye dokunmayan process-level test direct `MessageIndexing` aktivasyonunu, legacy `E_ACCESSDENIED` sinirini ve revoke sonrasi `REGDB_E_CLASSNOTREG` davranisini dogruluyor. Registry/service-install wiring'i ve authenticated `Application -> Settings` yolu henuz tamamlanmadi.
