...
Backlog'daki siradaki ana dilim: Administrator object modeline sinirli read-only `Application -> Rules` global-rule dilimiyle devam etmek. Korunmus legacy `Rules`/`Rule` COM kontratlari ve mevcut SQL store yeniden kullanilmali; authenticated global rule count/index/id lookup ve ayni read-only core scalar'lar `hm_rules` icinde `ruleaccountid = 0` verisinden gelmeli. Criteria/actions, execution, mutation ve derin davranislar acik `E_NOTIMPL` kalmali.
