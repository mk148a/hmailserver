...
Backlog'daki siradaki ana dilim: partial/fake bir COM object model yayinlamadan legacy `Application` authentication ve `Settings` contract bagimliliklarini haritalayip yeni service-process class-factory host uzerinde en kucuk legacy-complete authenticated `Application -> Settings -> MessageIndexing` yolunu tamamlamak. Direct CLSID activation access-denied kalmali; registry/type-library ve version-independent ProgID/CurVer wiring'i service-install diliminde tamamlanmali, runtime production-ready sayilmadan once SQL Server integration testi eklenmeli.
